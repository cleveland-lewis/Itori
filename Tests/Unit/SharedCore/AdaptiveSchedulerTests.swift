    ...
    final class AdaptiveSchedulerTests: XCTestCase {
        ...
        func testEnergyProfilePreferenceAffectsScheduling() {
            ...
            let constraints = AIScheduler.Constraints(
                horizonStart: date(2025, 12, 1, 0, 0),
                horizonEnd: date(2025, 12, 2, 23, 59),
                dayStartHour: 8,
                dayEndHour: 23,
                maxStudyMinutesPerDay: 600,
                maxStudyMinutesPerBlock: 180,
                minGapBetweenBlocksMinutes: 10,
                doNotScheduleWindows: [],
                energyProfile: (0 ..< 24).reduce(into: [:]) { $0[$1] = 0.5 }
            )

            var prefsMorning = SchedulerPreferences.default()
            for h in 0 ..< 24 {
                prefsMorning.learnedEnergyProfile[h] = 0.1
            }
            prefsMorning.learnedEnergyProfile[9] = 1.0

            var prefsNight = SchedulerPreferences.default()
            for h in 0 ..< 24 {
                prefsNight.learnedEnergyProfile[h] = 0.1
            }
            prefsNight.learnedEnergyProfile[21] = 1.0

            let resMorning = AIScheduler.generateSchedule(
                tasks: [task],
                fixedEvents: [],
                constraints: constraints,
                preferences: prefsMorning
            )
            let resNight = AIScheduler.generateSchedule(
                tasks: [task],
                fixedEvents: [],
                constraints: constraints,
                preferences: prefsNight
            )
        ...
        func testFeedbackUpdatesPreferences() {
            ...
            let fb1 = BlockFeedback(
                blockId: UUID(),
                taskId: UUID(),
                courseId: nil,
                type: .reading,
                start: date(2025, 12, 1, 9, 0),
                end: date(2025, 12, 1, 9, 30),
                completion: 1.0,
                action: .kept
            )
            let fb2 = BlockFeedback(
                blockId: UUID(),
                taskId: UUID(),
                courseId: nil,
                type: .reading,
                start: date(2025, 12, 1, 21, 0),
                end: date(2025, 12, 1, 21, 30),
                completion: 0.0,
                action: .deleted
            )
