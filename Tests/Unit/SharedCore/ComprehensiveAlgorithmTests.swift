...
struct ComprehensiveAlgorithmTests {
    ...
    @Test func assignmentPlanEngine_Determinism() async throws {
    ...
    @Test func assignmentPlanEngine_AllCategories() async throws {
        let dueDate = Date().addingTimeInterval(7 * 24 * 60 * 60)
    ...
    @Test func plannerEngine_SessionGeneration() async throws {
        let dueDate = Date().addingTimeInterval(7 * 24 * 60 * 60)
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
    ...
    @Test func plannerEngine_ScheduleIndexCalculation() async throws {
        let today = Date()
    ...
    @Test func plannerEngine_NoOverlappingSessions() async throws {
        let today = Date()
        ...
        for assignment in assignments {
            allSessions.append(contentsOf: PlannerEngine.generateSessions(
                for: assignment,
    ...
    @Test func blueprintGenerator_Determinism() async throws {
    ...
    @Test func blueprintGenerator_QuestionDistribution() async throws {
        let request = PracticeTestRequest(
            courseName: "Science",
    ...
    @Test func blueprintGenerator_DifficultyLevels() async throws {
    ...
    @Test func gradeCalculator_WeightedAverage() async throws {
        let courseId = UUID()
        let tasks = [
    ...
    @Test func gradeCalculator_NoGradedWork() async throws {
        let courseId = UUID()
        let tasks: [AppTask] = []
    ...
    @Test func gradeCalculator_GPACalculation() async throws {
    ...
    @Test func assignmentPlan_LinearChainSetup() async throws {
    ...
    @Test func assignmentPlan_CycleDetection() async throws {
        var plan = AssignmentPlan(assignmentId: UUID(), sequenceEnforcementEnabled: true)
        ...
        #expect(!cycle!.isEmpty, "Cycle path should contain step IDs")
    }

    @Test func assignmentPlan_TopologicalSort() async throws {
        var plan = AssignmentPlan(assignmentId: UUID(), sequenceEnforcementEnabled: true)
    ...
    @Test func assignmentPlan_QualityScore() async throws {
        let dueDate = Date().addingTimeInterval(7 * 24 * 60 * 60)
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
    ...
    @Test func assignmentPlan_ComplexityScore() async throws {
    ...
    @Test func planValidation_ValidPlan() async throws {
        let dueDate = Date().addingTimeInterval(7 * 24 * 60 * 60)
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
    ...
    @Test func planValidation_EmptyPlan() async throws {
    ...
    @Test func planValidation_InvalidSequence() async throws {
    ...
    @Test func performance_PlanGeneration() async throws {
        let dueDate = Date().addingTimeInterval(7 * 24 * 60 * 60)
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
    ...
    @Test func performance_SessionGeneration() async throws {
        let dueDate = Date().addingTimeInterval(7 * 24 * 60 * 60)
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
    ...
    @Test func performance_Scheduling() async throws {
        let dueDate = Date().addingTimeInterval(7 * 24 * 60 * 60)
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
    ...
    @Test func edgeCase_MinimalDuration() async throws {
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
            title: "Quick Task",
    ...
    @Test func edgeCase_ZeroDuration() async throws {
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
            title: "Zero Task",
    ...
    @Test func edgeCase_VeryLongDuration() async throws {
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
            title: "Long Project",
    ...
    @Test func edgeCase_ExtremeLongDuration() async throws {
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
    ...
    @Test func edgeCase_NearDueDate() async throws {
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
            title: "Urgent Task",
    ...
    @Test func edgeCase_PastDueDate() async throws {
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
            title: "Overdue Task",
    ...
    @Test func edgeCase_VeryFarFuture() async throws {
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
    ...
    @Test func edgeCase_SameSecondDueDate() async throws {
        let now = Date()
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
    ...
    @Test func edgeCase_TooManyAssignments() async throws {
        let today = Date()
        ...
        #expect(!result.overflow.isEmpty, "Should overflow when too many assignments")
        #expect(!result.scheduled.isEmpty, "Should schedule as many as possible")
    }

    @Test func edgeCase_NoAssignments() async throws {
    ...
    @Test func edgeCase_AllSameDueDate() async throws {
    ...
    @Test func edgeCase_NoAvailableTime() async throws {
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
            title: "Test",
        ...
        #expect(!result.overflow.isEmpty, "Should overflow when insufficient time")
    }

    @Test func edgeCase_AllLowEnergyTime() async throws {
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
            title: "Test",
        ...
        let result = PlannerEngine.scheduleSessions(
            sessions,
            settings: lowEnergyProfile,
            energyProfile: lowEnergyProfile
        )

        #expect(!result.scheduled.isEmpty, "Should still schedule even with low energy")
    }

    // MARK: Grade Calculator Edge Cases

    @Test func edgeCase_PerfectGrades() async throws {
        let courseId = UUID()
        let tasks = [
    ...
    @Test func edgeCase_FailingGrades() async throws {
        let courseId = UUID()
        let tasks = [
    ...
    @Test func edgeCase_UnequalWeights() async throws {
        let courseId = UUID()
        let tasks = [
    ...
    @Test func edgeCase_ZeroWeight() async throws {
        let courseId = UUID()
        let tasks = [
    ...
    @Test func edgeCase_ExtraCredit() async throws {
        let courseId = UUID()
        let tasks = [
    ...
    @Test func edgeCase_SelfReferencingDependency() async throws {
        var plan = AssignmentPlan(assignmentId: UUID(), sequenceEnforcementEnabled: true)
    ...
    @Test func edgeCase_LongDependencyChain() async throws {
        var plan = AssignmentPlan(assignmentId: UUID(), sequenceEnforcementEnabled: true)
    ...
    @Test func edgeCase_ComplexDependencyGraph() async throws {
        var plan = AssignmentPlan(assignmentId: UUID(), sequenceEnforcementEnabled: true)
    ...
    @Test func edgeCase_MultipleDependencyCycles() async throws {
        var plan = AssignmentPlan(assignmentId: UUID(), sequenceEnforcementEnabled: true)
    ...
    @Test func edgeCase_ExamWithMinimalLeadTime() async throws {
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
    ...
    @Test func edgeCase_ProjectWithoutCustomPlan() async throws {
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
    ...
    @Test func edgeCase_ReadingWithSinglePage() async throws {
        let assignment = Assignment(
            id: UUID(),
            courseId: nil,
