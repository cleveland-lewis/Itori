    ...
    final class AISchedulerTests: XCTestCase {
        ...
        func testDailyLimitRespected() {
            let t = AIScheduler.Task(
                id: UUID(),
                title: "Long",
                courseId: nil,
                due: date(2025, 12, 3, 0, 0),
            ...
            let res = AIScheduler.generateSchedule(
                ...
                options: AIScheduler.SchedulerOptions()
            )
            ...
            for (_, v) in minsByDay {
                XCTAssertLessThanOrEqual(v, 120)
            }
        }
        ...
        func testDoNotScheduleWindowsRespected() {
            ...
            let constraints = AIScheduler.Constraints(
                ...
                maxStudyMinutesPerBlock: 180,
                minGapBetweenBlocksMinutes: 10,
                doNotScheduleWindows: [blockWindow],
                energyProfile: (0 ..< 24).reduce(into: [:]) { $0[$1] = 0.5 }
            )
            let res = AIScheduler.generateSchedule(
                tasks: [t],
