...
class RecurringEventStore {
    ...
    func createSeriesWithEndDate(title: String, start: Date, end: Date, frequency: EKRecurrenceFrequency) -> RecurringSeries {
        let series = RecurringSeries(
            identifier: UUID().uuidString,
            title: title,
            start: start,
    ...
    func createMonthlySeriesOnDay(title: String, start: Date, dayOfMonth: Int, occurrences: Int) -> RecurringSeries {
        return createCustomSeries(
            title: title,
            start: start,
            frequency: .monthly,
    ...
    func createCustomSeries(title: String, start: Date, frequency: EKRecurrenceFrequency, interval: Int, count: Int) -> RecurringSeries {
        let series = RecurringSeries(
            identifier: UUID().uuidString,
            title: title,
            start: start,
    ...
    func getDetachedEvent(seriesID: String, originalDate: Date) -> DetachedEvent? {
        return detachedEvents[seriesID]?[originalDate]
    }
...
class RecurringSeries {
    ...
    init(identifier: String, title: String, start: Date, frequency: EKRecurrenceFrequency, interval: Int, recurrenceEnd: EKRecurrenceEnd?) {
    ...
    var occurrences: [SeriesOccurrence] {
        ...
        if let end = recurrenceEnd {
            if let count = end.occurrenceCount {
                ...
                maxOccurrences = 100 // arbitrary limit for date-based end
            } else {
                maxOccurrences = 100
            }
        } else {
...
class RecurrenceSeriesManager {
    ...
    func deleteOccurrence(seriesID: String, occurrenceDate: Date, span: EKSpan) async throws {
        ...
        if span == .thisEvent {
            ...
            if let index = occurrences.firstIndex(where: { Calendar.current.isDate($0.start, inSameDayAs: occurrenceDate) }),
               index > 0 {
