...
final class CalendarRecurrenceTests: XCTestCase {
    ...
    func testWeeklyRecurrenceMultipleDays() {
        ...
        let weekdays = rule.daysOfTheWeek?.map(\.dayOfTheWeek) ?? []
    ...
    func testWeekdayMaskConversion() {
        ...
        let selectedDays = weekdaySelection.filter(\.value).compactMap { index, _ -> EKRecurrenceDayOfWeek? in
        ...
        let weekdays = selectedDays.map(\.dayOfTheWeek.rawValue)
    ...
    func testMultipleAlarmsOrdering() {
        ...
        for (index, alarm) in alarms.enumerated() {
            XCTAssertEqual(
                ...
                TimeInterval(offsets[index] * 60),
                "Alarm \(index) offset should match"
            )
        }
    }
