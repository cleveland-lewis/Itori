...
public enum AccessibilityTestHelpers {
    // MARK: - VoiceOver Testing

    /// Verify that a view has an accessibility label
    public static func assertHasLabel(_: some View, file _: StaticString = #file, line _: UInt = #line) {
    ...
    public static func assertHasLabelAndHint(_: some View, file _: StaticString = #file, line _: UInt = #line) {
    ...
    public static func contrastRatio(foreground: Color, background: Color) -> Double {
            ...
            guard let fgColor = NSColor(foreground).cgColor,
                  let bgColor = NSColor(background).cgColor
            else {
                return 1.0
            }
        #else
            guard let fgColor = UIColor(foreground).cgColor,
                  let bgColor = UIColor(background).cgColor
            else {
                return 1.0
            }
        #endif
    ...
    public static func assertMeetsMinimumTouchTarget(
        size: CGSize,
        file _: StaticString = #file,
        line _: UInt = #line
    ) -> Bool {
    ...
    public static func assertAnimationReducedForReduceMotion(
        ...
        reducedDuration < normalDuration * 0.3
    }
}
