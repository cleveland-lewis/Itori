...
struct AISettingsView: View {
    ...
    var body: some View {
        Form {
                ...
                Section {
                    Label {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("LLM Features Disabled")
                                .font(.headline)
                            Text("LLM features are disabled in Privacy settings. Enable LLM assistance in Settings → Privacy to use these features.")
                                .font(.caption)
                        }
                    } icon: {
            ...
            Section("LLM Mode") {
                ...
                .disabled(!settings.enableLLMAssistance)
                .onChange(of: settings.aiMode) { _, _ in
                    settings.save()
                    LOG_SETTINGS(.info, "LLMModeChanged", "LLM mode changed", metadata: ["mode": settings.aiMode.rawValue])
                }
            ...
            .opacity(settings.enableLLMAssistance ? 1.0 : 0.5)
            ...
            Section("Privacy") {
                Label {
                    Text("All LLM requests are logged for debugging")
                } icon: {
    ...
    private var modeDescription: String {
        ...
        case .auto:
            return "Automatically selects the best available LLM provider. Prefers Apple Intelligence when available, falls back to local model."
        case .appleIntelligenceOnly:
            return "Only use Apple Intelligence. LLM features will be disabled if Apple Intelligence is unavailable."
        case .localOnly:
            return "Only use the local on-device model. Guaranteed no network access."
        case .byoProvider:
            return "Use your own LLM provider (OpenAI, Anthropic, or custom endpoint). Requires API key and network access."
        }
    }
    ...
    private func downloadModel() {
        ...
        Task {
            ...
            await MainActor.run {
                ...
                LOG_SETTINGS(.info, "ModelDownloaded", "Local LLM model downloaded")
            }
        }
    }
