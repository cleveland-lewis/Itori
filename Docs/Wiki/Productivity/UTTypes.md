...
## Overview
- Itori exports three custom uniform type identifiers: `com.roots.assignment`, `com.roots.course`, and `com.roots.session`.
...
- The shared UTIs make it possible to drop assignments into other apps that understand `public.data` (the UTI’s conformance) while still letting Itori decode the structured JSON payload.
...
- Info.plist now declares each UTI with a description matching the entity (“Itori Assignment”, “Itori Course”, “Itori Session”) and `UTTypeConformsTo` `public.data`.
...
- The `com.roots.session` UTI exists for future planner/timer session drags, ensuring third-party hosts can detect Itori session data when it becomes available.
...
- `ItoriApp.xcodeproj` build settings now expose each UTI through `INFOPLIST_KEY_UTExportedTypeDeclarations` so other systems can discover them at runtime.
