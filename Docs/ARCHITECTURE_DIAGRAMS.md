# Itori Multi-Target Architecture Diagram

```
┌─────────────────────────────────────────────────────────────────────┐
│                         Itori.xcodeproj                              │
└─────────────────────────────────────────────────────────────────────┘
                                  │
                  ┌───────────────┴───────────────┐
                  │                               │
                  ▼                               ▼
┌─────────────────────────────┐   ┌─────────────────────────────┐
│      ItoriApp Target        │   │     ItoriMac Target         │
│       (iOS/iPadOS)          │   │        (macOS)              │
└─────────────────────────────┘   └─────────────────────────────┘
│                               │   │                               │
│  ├─ ItoriApp.swift           │   │  ├─ ItoriMacApp.swift        │
│  ├─ Info.plist               │   │  ├─ Info.plist               │
│  ├─ Itori.entitlements       │   │  ├─ ItoriMac.entitlements    │
│  │                            │   │  │                            │
│  ├─ Views/                   │   │  ├─ Views/                   │
│  │   ├─ IOSRootView.swift    │   │  │   ├─ MacRootView.swift    │
│  │   ├─ IOSTabBar.swift      │   │  │   ├─ MacSidebar.swift     │
│  │   ├─ Dashboard/           │   │  │   ├─ Dashboard/           │
│  │   ├─ Calendar/            │   │  │   ├─ Calendar/            │
│  │   ├─ Planner/             │   │  │   ├─ Planner/             │
│  │   └─ ...                  │   │  │   └─ ...                  │
│  │                            │   │  │                            │
│  ├─ PlatformExtensions/      │   │  ├─ PlatformExtensions/      │
│  │   ├─ Color+iOS.swift      │   │  │   ├─ Color+macOS.swift    │
│  │   ├─ Assignment+iOS.swift │   │  │   ├─ Assignment+macOS.swift│
│  │   └─ EventKit+iOS.swift   │   │  │   └─ EventKit+macOS.swift │
│  │                            │   │  │                            │
│  ├─ DependencyInjection/     │   │  ├─ DependencyInjection/     │
│  │   └─ AppDependencies.swift│   │  │   └─ AppDependencies.swift│
│  │                            │   │  │                            │
│  └─ Assets.xcassets/         │   │  └─ Assets.xcassets/         │
│                               │   │                               │
└───────────────┬───────────────┘   └───────────────┬───────────────┘
                │                                   │
                │   ┌───────────────────────────┐   │
                └───►     ItoriShared (SPM)     ◄───┘
                    │   Swift Package Module    │
                    └───────────────────────────┘
                                  │
        ┌─────────────────────────┼─────────────────────────┐
        │                         │                         │
        ▼                         ▼                         ▼
┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐
│     Models/      │  │    Services/     │  │   Utilities/     │
├──────────────────┤  ├──────────────────┤  ├──────────────────┤
│ Course.swift     │  │ Protocols/       │  │ DateHelpers.swift│
│ Assignment.swift │  │   Persistence-   │  │ Formatters.swift │
│ Exam.swift       │  │   Service.swift  │  │ Validation.swift │
│ Schedule.swift   │  │                  │  │ Constants.swift  │
│ Semester.swift   │  │ AssignmentPlan-  │  └──────────────────┘
│ Grade.swift      │  │   Engine.swift   │
│ PlanStep.swift   │  │                  │  ┌──────────────────┐
└──────────────────┘  │ Scheduler-       │  │  DesignTokens/   │
                      │   Adaptation.swift│  ├──────────────────┤
                      │                  │  │ Spacing.swift    │
                      │ AutoScheduler-   │  │ Typography.swift │
                      │   .swift         │  │ BorderRadius.swift│
                      │                  │  │ Layout.swift     │
                      └──────────────────┘  └──────────────────┘

                      ┌──────────────────┐  ┌──────────────────┐
                      │  Persistence/    │  │  Localization/   │
                      ├──────────────────┤  ├──────────────────┤
                      │ Persistence-     │  │ Localization-    │
                      │   Container.swift│  │   Keys.swift     │
                      │ CoreDataStack-   │  └──────────────────┘
                      │   .swift         │
                      │ Persistence-     │  ┌──────────────────┐
                      │   Factory.swift  │  │ Dependency-      │
                      └──────────────────┘  │ Injection/       │
                                            ├──────────────────┤
                                            │ AppContainer-    │
                                            │   .swift         │
                                            └──────────────────┘
```

## Data Flow Diagram

```
┌─────────────────────────────────────────────────────────────────┐
│                    App Launch (iOS/macOS)                        │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│              Platform-Specific App Entry Point                   │
│                                                                  │
│  iOS: ItoriApp.swift        macOS: ItoriMacApp.swift           │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│              Create Platform-Specific Dependencies               │
│                                                                  │
│  let deps = AppDependencies.shared                              │
│  let persistence = IOSPersistenceService() // or Mac version    │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│              Initialize Shared Container (ItoriShared)           │
│                                                                  │
│  let container = AppContainer(persistenceService: persistence)  │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│               Inject Container into Root View                    │
│                                                                  │
│  IOSRootView().environmentObject(container)                     │
│  MacRootView().environmentObject(container)                     │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                Load Data from Persistence                        │
│                                                                  │
│  Task { await container.loadData() }                            │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                    Views Render with Data                        │
│                                                                  │
│  - DashboardView uses container.courses                         │
│  - CalendarView uses container.assignments                      │
│  - Platform extensions provide colors/formatting                │
└─────────────────────────────────────────────────────────────────┘
```

## Color Architecture Pattern

```
┌─────────────────────────────────────────────────────────────────┐
│                  ItoriShared (Cross-Platform)                    │
│                                                                  │
│  struct Course {                                                │
│      let id: UUID                                               │
│      var title: String                                          │
│      var colorHex: String  // ← Stored as hex string           │
│  }                                                              │
│                                                                  │
│  enum AssignmentUrgency {                                       │
│      case low, medium, high, critical                           │
│      var label: String { "Low" }  // ← No Color here           │
│  }                                                              │
└─────────────────────────────────────────────────────────────────┘
                              │
                ┌─────────────┴─────────────┐
                │                           │
                ▼                           ▼
┌───────────────────────────┐   ┌───────────────────────────┐
│  iOS Platform Extension   │   │  macOS Platform Extension │
│                           │   │                           │
│  extension Course {       │   │  extension Course {       │
│      var color: Color {   │   │      var color: Color {   │
│          Color(hex:       │   │          Color(hex:       │
│            colorHex)      │   │            colorHex)      │
│      }                    │   │      }                    │
│  }                        │   │  }                        │
│                           │   │                           │
│  extension Assignment-    │   │  extension Assignment-    │
│    Urgency {              │   │    Urgency {              │
│      var color: Color {   │   │      var color: Color {   │
│          switch self {    │   │          switch self {    │
│          case .low: .gray │   │          case .low: .gray │
│          case .high: .red │   │          case .high: .red │
│          }                │   │          }                │
│      }                    │   │      }                    │
│  }                        │   │  }                        │
└───────────────────────────┘   └───────────────────────────┘
                │                           │
                └─────────────┬─────────────┘
                              ▼
                    ┌──────────────────┐
                    │  SwiftUI Views   │
                    │                  │
                    │  course.color    │
                    │  urgency.color   │
                    └──────────────────┘
```

## Dependency Injection Flow

```
┌─────────────────────────────────────────────────────────────────┐
│                      ItoriShared/DI/                             │
│                                                                  │
│  protocol PersistenceService: Actor {                           │
│      func saveCourse(_ course: Course) async throws             │
│      func fetchCourses() async throws -> [Course]               │
│  }                                                              │
│                                                                  │
│  @MainActor                                                     │
│  class AppContainer: ObservableObject {                         │
│      let persistenceService: any PersistenceService             │
│      @Published var courses: [Course] = []                      │
│      @Published var assignments: [Assignment] = []              │
│                                                                  │
│      init(persistenceService: any PersistenceService) {         │
│          self.persistenceService = persistenceService           │
│      }                                                          │
│  }                                                              │
└─────────────────────────────────────────────────────────────────┘
                              │
                ┌─────────────┴─────────────┐
                │                           │
                ▼                           ▼
┌───────────────────────────┐   ┌───────────────────────────┐
│  iOS Implementation       │   │  macOS Implementation     │
│                           │   │                           │
│  actor IOSPersistence-    │   │  actor MacPersistence-    │
│    Service: Persistence-  │   │    Service: Persistence-  │
│    Service {              │   │    Service {              │
│                           │   │                           │
│      func saveCourse(...) │   │      func saveCourse(...) │
│      { /* CoreData */ }   │   │      { /* CoreData */ }   │
│  }                        │   │  }                        │
│                           │   │                           │
│  class AppDependencies {  │   │  class AppDependencies {  │
│      static let shared =  │   │      static let shared =  │
│        AppDependencies()  │   │        AppDependencies()  │
│                           │   │                           │
│      let container:       │   │      let container:       │
│        AppContainer       │   │        AppContainer       │
│                           │   │                           │
│      init() {             │   │      init() {             │
│          let persist =    │   │          let persist =    │
│            IOSPersist..() │   │            MacPersist..() │
│          container =      │   │          container =      │
│            AppContainer(  │   │            AppContainer(  │
│              persist)     │   │              persist)     │
│      }                    │   │      }                    │
│  }                        │   │  }                        │
└───────────────────────────┘   └───────────────────────────┘
                │                           │
                ▼                           ▼
┌───────────────────────────┐   ┌───────────────────────────┐
│  @main                    │   │  @main                    │
│  struct ItoriApp: App {   │   │  struct ItoriMacApp: App {│
│      @StateObject var     │   │      @StateObject var     │
│        container =        │   │        container =        │
│        AppDependencies    │   │        AppDependencies    │
│          .shared          │   │          .shared          │
│          .container       │   │          .container       │
│  }                        │   │  }                        │
└───────────────────────────┘   └───────────────────────────┘
```

## Build Process Flow

```
┌─────────────────────────────────────────────────────────────────┐
│                     Xcode Build Process                          │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                   Build ItoriShared Package                      │
│                                                                  │
│  1. Compile Models/*.swift                                      │
│  2. Compile Services/*.swift                                    │
│  3. Compile Utilities/*.swift                                   │
│  4. Link into ItoriShared.framework                             │
└─────────────────────────────────────────────────────────────────┘
                              │
                ┌─────────────┴─────────────┐
                │                           │
                ▼                           ▼
┌───────────────────────────┐   ┌───────────────────────────┐
│  Build iOS Target         │   │  Build macOS Target       │
│                           │   │                           │
│  1. Link ItoriShared      │   │  1. Link ItoriShared      │
│  2. Compile Views/*.swift │   │  2. Compile Views/*.swift │
│  3. Compile Platform-     │   │  3. Compile Platform-     │
│     Extensions/*.swift    │   │     Extensions/*.swift    │
│  4. Compile ItoriApp.swift│   │  4. Compile ItoriMac.swift│
│  5. Link frameworks       │   │  5. Link frameworks       │
│  6. Code sign             │   │  6. Code sign             │
│  7. Generate .app         │   │  7. Generate .app         │
└───────────────────────────┘   └───────────────────────────┘
                │                           │
                ▼                           ▼
┌───────────────────────────┐   ┌───────────────────────────┐
│    ItoriApp.app           │   │    ItoriMac.app           │
│  (iOS/iPadOS)             │   │    (macOS)                │
└───────────────────────────┘   └───────────────────────────┘
```

## Key Principles Summary

```
┌─────────────────────────────────────────────────────────────────┐
│                    Code Placement Rules                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ItoriShared (Cross-Platform):                                  │
│    ✓ Domain models (Course, Assignment, etc.)                   │
│    ✓ Business logic (planning, scheduling)                      │
│    ✓ Service protocols                                          │
│    ✓ Utilities (Date, String extensions)                        │
│    ✓ Design tokens (spacing, radii)                             │
│    ✓ Localization keys                                          │
│    ✗ SwiftUI Views                                              │
│    ✗ Color/NSColor/UIColor types                                │
│    ✗ Platform-specific APIs                                     │
│                                                                  │
│  Platform Targets (iOS/macOS):                                  │
│    ✓ SwiftUI views and navigation                               │
│    ✓ Platform-specific extensions (colors)                      │
│    ✓ Platform capability integrations                           │
│    ✓ App entry points                                           │
│    ✓ Service implementations                                    │
│    ✗ Business logic duplication                                 │
│    ✗ Model definitions                                          │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

**This architecture ensures maximum code sharing while maintaining platform-native UIs! 🎯**
