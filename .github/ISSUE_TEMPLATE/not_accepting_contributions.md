---
name: ⚠️ Contributions Not Accepted
about: This template explains why contributions cannot be accepted
title: 'Please read: This repository does not accept contributions'
labels: invalid
assignees: ''

---

## ⚠️ This Repository Does NOT Accept Contributions

This issue template exists to make it clear: **this is not an open source project.**

### What This Repository Is

This is **proprietary software** made available in source-visible form for:
- Security auditing and transparency
- Educational review (viewing only)
- User trust and accountability

### What You CANNOT Do

You may NOT:
- ❌ Submit feature requests
- ❌ Submit bug fixes via pull request
- ❌ Contribute code or documentation
- ❌ Fork for development purposes
- ❌ Use this code in other projects

### License

This software is licensed under a **Proprietary Source-Available License**.

See: [LICENSE](https://github.com/cleveland-lewis/Roots/blob/main/LICENSE)

All rights reserved. No permission is granted to use, modify, or distribute this software.

---

### If You Want to Report a Security Issue

Use private security advisories:  
👉 https://github.com/cleveland-lewis/Roots/security/advisories/new

See: [SECURITY.md](https://github.com/cleveland-lewis/Roots/blob/main/SECURITY.md)

---

### Questions About Licensing

Contact Cleveland Lewis (see repository profile)

---

**This issue will be closed immediately.**
