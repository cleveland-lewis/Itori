    ...
    struct WatchTimerProvider: TimelineProvider {
        ...
        func getSnapshot(in _: Context, completion: @escaping (WatchTimerEntry) -> Void) {
            completion(WatchTimerEntry(date: Date(), state: loadState()))
        }

        func getTimeline(in _: Context, completion: @escaping (Timeline<WatchTimerEntry>) -> Void) {
        ...
        private func loadState() -> WatchTimerStateSnapshot? {
            guard let defaults = UserDefaults(suiteName: AppGroupConstants.identifier),
                  ...
                  let state = try? JSONDecoder().decode(WatchTimerStateSnapshot.self, from: data)
            else {
                return nil
            }
            return state
