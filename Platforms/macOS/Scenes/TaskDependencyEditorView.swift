    ...
    struct TaskDependencyEditorView: View {
            ...
            HStack(spacing: DesignSystem.Layout.spacing.medium) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(NSLocalizedString(
                        "taskdependencyeditor.task.dependencies",
                ...
                if let plan {
                    Toggle(isOn: Binding(
                        ...
                        Label(
                            NSLocalizedString(
                                "taskdependencyeditor.label.enforce.task.order",
                                value: "Enforce Task Order",
                                comment: "Enforce Task Order"
                            ),
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: DesignSystem.Layout.spacing.large) {
                    ...
                    VStack(alignment: .leading, spacing: DesignSystem.Layout.spacing.small) {
                        Text(NSLocalizedString(
                            "taskdependencyeditor.task.order",
                            value: "Task Order",
                            comment: "Task Order"
                        ))
                        .font(.headline)
        ...
        private var enforcementBanner: some View {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(NSLocalizedString(
                        "taskdependencyeditor.task.order.enforced",
                        value: "Task Order Enforced",
                        comment: "Task Order Enforced"
                    ))
                    .font(.subheadline.weight(.semibold))
                    Text(NSLocalizedString(
                        "taskdependencyeditor.tasks.must.be.completed.in",
                        value: "Tasks must be completed in sequence. Drag to reorder.",
                        comment: "Tasks must be completed in sequence. Drag to reord..."
                    ))
                    .font(.caption)
                    .foregroundStyle(.secondary)
                }
                ...
                Button(NSLocalizedString(
                    "taskdependencyeditor.button.clear.dependencies",
                    value: "Clear Dependencies",
                    comment: "Clear Dependencies"
                )) {
                    clearDependencies()
                }
            ...
            HStack(spacing: DesignSystem.Layout.spacing.medium) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack(spacing: 12) {
                        Label(
                            NSLocalizedString(
                                "taskdependencyeditor.label.stepestimatedminutes.min",
                        ...
                        if step.isCompleted {
                            Label(
                                NSLocalizedString(
                                    "taskdependencyeditor.label.completed",
                                    value: "Completed",
                                    comment: "Completed"
                                ),
                            ...
                            Label(
                                NSLocalizedString(
                                    "taskdependencyeditor.label.blocked",
                                    value: "Blocked",
                                    comment: "Blocked"
                                ),
        ...
        private var infoSection: some View {
            VStack(alignment: .leading, spacing: 12) {
                Text(NSLocalizedString(
                    "taskdependencyeditor.how.dependencies.work",
                    value: "How Dependencies Work",
                    comment: "How Dependencies Work"
                ))
                .font(.headline)
        ...
        private var emptyStateView: some View {
            VStack(spacing: 16) {
                Image(systemName: "list.bullet.clipboard")
                    ...
                    .foregroundStyle(.secondary)
                Text(NSLocalizedString(
                    "taskdependencyeditor.no.tasks.in.plan",
                    value: "No Tasks in Plan",
                    comment: "No Tasks in Plan"
                ))
                .font(.title3.weight(.semibold))
                Text(NSLocalizedString(
                    "taskdependencyeditor.create.a.plan.for.this",
                    value: "Create a plan for this assignment to manage task dependencies",
                    comment: "Create a plan for this assignment to manage task d..."
                ))
                .font(.subheadline)
                .foregroundStyle(.secondary)
        ...
        private var noPlanView: some View {
            VStack(spacing: 16) {
                Image(systemName: "calendar.badge.exclamationmark")
                    ...
                    .foregroundStyle(.secondary)
                Text(NSLocalizedString(
                    "taskdependencyeditor.no.plan.found",
                    value: "No Plan Found",
                    comment: "No Plan Found"
                ))
                .font(.title3.weight(.semibold))
                Text(NSLocalizedString(
                    "taskdependencyeditor.create.an.assignment.plan.to",
                    value: "Create an assignment plan to enable task dependencies",
                    comment: "Create an assignment plan to enable task dependenc..."
                ))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
