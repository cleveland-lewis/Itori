    ...
    struct CoursesDashboardRootView: View {
        ...
        private var emptyDetailState: some View {
            VStack(spacing: 16) {
                ...
                Text(NSLocalizedString(
                    "coursesdashboardroot.select.a.course",
                ...
                Text(NSLocalizedString(
                    "coursesdashboardroot.choose.a.course.from.the.sidebar.to.view.details",
                    value: "Choose a course from the sidebar to view details",
                    comment: "Choose a course from the sidebar to view details"
                ))
                .font(.subheadline)
        ...
        private var loadingState: some View {
            VStack(spacing: 12) {
                ProgressView()
                Text(NSLocalizedString(
                    "coursesdashboardroot.loading.course.analytics",
                    value: "Loading course analytics…",
                    comment: "Loading course analytics…"
                ))
                .font(.subheadline)
                .foregroundStyle(.secondary)
            }
    ...
    private extension CoursesDashboardRootView {
        func refreshFromStores() {
            ...
            courses = activeCourses.map { course in
                ...
                let completedCount = matchingTasks.filter(\.isCompleted).count
