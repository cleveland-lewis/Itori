    ...
    struct EditCardSheet: View {
            ...
            VStack(spacing: 20) {
                ...
                HStack {
                    Button(NSLocalizedString(
                        "ui.button.reset.progress",
                        value: "Reset Progress",
                        comment: "Reset Progress"
                    )) {
                        resetProgress()
                    }
                    .buttonStyle(.itoriLiquidProminent)
        ...
        private func saveCard() {
            guard let deckIndex = manager.decks.firstIndex(where: { $0.id == deckId }),
                  let cardIndex = manager.decks[deckIndex].cards.firstIndex(where: { $0.id == card.id })
            else {
                return
            }
                ...
                Form {
                    ...
                    Section {
                        Button(role: .destructive) {
                            ...
                            Label(
                                NSLocalizedString("ui.label.delete.deck", value: "Delete Deck", comment: "Delete Deck"),
                                systemImage: "trash"
                            )
                ...
                if text.isEmpty {
                    ...
                    VStack(spacing: 16) {
                        ...
                        Text(NSLocalizedString(
                            "ui.no.cards.to.export",
                            value: "No Cards to Export",
                            comment: "No Cards to Export"
                        ))
                        .font(.title3.weight(.semibold))

                        Text(NSLocalizedString(
                            "ui.add.some.cards.to.this",
                            value: "Add some cards to this deck to export them to Anki format.",
                            comment: "Add some cards to this deck to export them to Anki..."
                        ))
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        HStack {
                            Text(NSLocalizedString(
                                "ui.csv.format.ready.for.anki.import",
                                value: "CSV format ready for Anki import",
                                comment: "CSV format ready for Anki import"
                            ))
                            .font(.subheadline)
                            .foregroundStyle(.secondary)
                            ...
                            Text(String(
                                format: NSLocalizedString(
                                    "flashcards.export.cards_count",
                                    value: "%d cards",
                                    comment: "Exported cards count"
                                ),
