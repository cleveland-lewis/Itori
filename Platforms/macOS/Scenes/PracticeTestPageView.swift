    ...
    struct PracticeTestPageView: View {
                ...
                HStack {
                    HStack(spacing: 8) {
                        Text(NSLocalizedString(
                            "practicetest.scheduled.tests",
                            value: "Scheduled Tests",
                            comment: "Scheduled Tests"
                        ))
                        .font(.title2.weight(.semibold))

                        if !scheduledTestsStore.scheduledTests.filter({ $0.status != .archived }).isEmpty {
                            Text(
                                verbatim: "\(scheduledTestsStore.scheduledTests.filter({ $0.status != .archived }).count)"
                            )
                    ...
                    Button {
                        ...
                        Label(
                            NSLocalizedString(
                                "practicetest.label.new.practice.test",
        ...
        private var emptyStateView: some View {
            VStack(spacing: 20) {
                ...
                Button {
                    ...
                    Label(
                        NSLocalizedString(
                            "practicetest.label.create.practice.test",
                            value: "Create Practice Test",
                            comment: "Create Practice Test"
                        ),
                ...
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        HStack(spacing: 8) {
                            Label(
                                NSLocalizedString(
                                    "practicetest.label.testquestioncount.questions",
                                    value: "\(test.questionCount) questions",
                                    comment: "\(test.questionCount) questions"
                                ),
                                systemImage: "number"
                            )
            ...
            .contextMenu {
                ...
                if test.status == .submitted {
                    Button(NSLocalizedString(
                        "practicetest.button.review.answers",
                        value: "Review Answers",
                        comment: "Review Answers"
                    )) {
                        practiceStore.currentTest = test
                    }
                }
                ...
                if test.status == .ready {
                    Button(NSLocalizedString(
                        "practicetest.button.start.test",
                        value: "Start Test",
                        comment: "Start Test"
                    )) {
                ...
                if test.status == .failed {
                    Button(NSLocalizedString(
                        "practicetest.button.retry.generation",
        ...
        private func failedView(test: PracticeTest) -> some View {
            VStack(spacing: 24) {
                ...
                Text(NSLocalizedString(
                    "practicetest.generation.failed",
