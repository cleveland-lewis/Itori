    ...
    struct AssignmentsView: View {
        enum Filter: String, CaseIterable, Identifiable {
            ...
            var displayName: String {
                switch self {
                case .all: NSLocalizedString("assignments.filter.all", comment: "")
                case .dueSoon: NSLocalizedString("assignments.filter.due_soon", comment: "")
                case .overdue: NSLocalizedString("assignments.filter.overdue", comment: "")
                case .personal: "Personal" // NEW
                }
            }
        }
        ...
        private var filteredTasks: [AppTask] {
            ...
            case .personal:
                return tasks.filter(\.isPersonal) // NEW: Filter personal tasks
            }
        }
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: DesignSystem.Spacing.large) {
                    ...
                    LazyVStack(alignment: .leading, spacing: DesignSystem.Spacing.large) {
                        // Due Today
                        Section(header: Text(NSLocalizedString("assignments.section.due_today", comment: ""))
                            .font(DesignSystem.Typography.body))
                        {
                            let tasks = assignmentsStore.tasks
                                .filter { $0.due != nil && Calendar.current.isDateInToday($0.due!) }
                            if tasks.isEmpty {
                                AppCard {
                        ...
                        // Due This Week
                        Section(header: Text(NSLocalizedString("assignments.section.due_this_week", comment: ""))
                            .font(DesignSystem.Typography.body))
                        {
                            let tasks = assignmentsStore.tasks.filter { t in
                                if let d = t.due { return Calendar.current.isDate(
                                    d,
                                    equalTo: Date(),
                        ...
                        if filter == .all || filter == .personal {
                            Section(header: HStack {
                                Text(NSLocalizedString(
                                    "assignments.personal.tasks",
                                    value: "Personal Tasks",
                                    comment: "Personal Tasks"
                                ))
                                .font(DesignSystem.Typography.body)
                                ...
                                if personalTasks.isEmpty {
                                    AppCard {
                                        VStack(spacing: DesignSystem.Spacing.small) {
                                            ...
                                            .font(DesignSystem.Typography.title)
                                            Text(NSLocalizedString(
                                                "assignments.create.personal.tasks.not.tied.to.any.course",
                                                value: "Create personal tasks not tied to any course",
                                                comment: "Create personal tasks not tied to any course"
                                            ))
                                            .font(DesignSystem.Typography.body)
                                            .foregroundStyle(.secondary)
                                        }
                        ...
                        // Upcoming
                        Section(header: Text(NSLocalizedString("assignments.section.upcoming", comment: ""))
                            .font(DesignSystem.Typography.body))
                        {
                        ...
                        // Overdue
                        Section(header: Text(NSLocalizedString("assignments.filter.overdue", comment: ""))
                            .font(DesignSystem.Typography.body))
                        {
                            let tasks = assignmentsStore.tasks.filter { t in
