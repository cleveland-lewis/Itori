    ...
    struct CoursesPageView: View {
        ...
        private func vm(from course: Course) -> CoursePageCourse {
            let semesterName = coursesStore.semesters.first(where: { $0.id == course.semesterId })?
                .name ?? NSLocalizedString(
                    "courses.default.current_term",
                    comment: ""
                )
        ...
        private func hasParsedAssignments(for course: CoursePageCourse) -> Bool {
            !parsingStore.parsedAssignmentsByCourse(course.id).isEmpty
        }
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: ItariSpacing.s) {
                    ...
                    VStack(spacing: ItariSpacing.s) {
                        ...
                        SidebarWidgetTile(
                            label: NSLocalizedString("courses.widget.total_credits", comment: ""),
    ...
    struct CoursesPageDetailView: View {
        ...
        var onReviewParsedAssignments: (() -> Void)?
                ...
                if course.meetingTimes.isEmpty {
                    ...
                    VStack(alignment: .leading, spacing: ItariSpacing.s) {
                        ForEach(course.meetingTimes) { meeting in
                            VStack(alignment: .leading, spacing: 4) {
                                Text(String(
                                    format: NSLocalizedString("courses.meeting.day_time", comment: ""),
                        ...
                        ForEach(syllabus.categories) { category in
                            VStack(alignment: .leading, spacing: 6) {
                                HStack {
                                    ...
                                    Spacer()
                                    Text(String(
                                        format: NSLocalizedString("courses.meeting.weight", comment: ""),
                                        Int(category.weight)
                                    ))
                                    .font(.caption.weight(.semibold))
                                }
                    ...
                    if !syllabus.notes.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                        VStack(alignment: .leading, spacing: 6) {
                            Text(NSLocalizedString(
                                "courses.section.notes",
                                value: "Notes",
                                comment: "Course notes header"
                            ))
                            .font(.caption.weight(.semibold))
                            .foregroundStyle(.secondary)
                            ScrollView {
        ...
        var body: some View {
            ZStack {
                ...
                if let current = gradeInfo.currentPercentage {
                    ...
                    VStack(spacing: 2) {
                        Text(String(
                            format: NSLocalizedString("courses.grade.percent_display", comment: ""),
                            Int(current)
                        ))
                        .font(DesignSystem.Typography.body)
                    ...
                    Button {
                        ...
                        Circle()
                            ...
                            .overlay(
                                Circle()
                                    .stroke(
                                        selected == tag ? Color.accentColor : Color.separatorColor.opacity(0.12),
                                        lineWidth: selected == tag ? 3 : 1
                                    )
                            )
                    }
                    .buttonStyle(.plain)
    ...
    struct CourseEditorSheet: View {
        ...
        private var courseSection: some View {
            VStack(alignment: .leading, spacing: ItariSpacing.m) {
                ...
                .validationHint(
                    isInvalid: code.trimmingCharacters(in: .whitespaces).isEmpty,
                ...
                .validationHint(
                    isInvalid: title.trimmingCharacters(in: .whitespaces).isEmpty,
        ...
        private var actionBar: some View {
            HStack {
                ...
                Button(isNew ? NSLocalizedString("courses.form.button.create", comment: "") : NSLocalizedString(
                    "courses.form.button.save",
                    comment: ""
                )) {
                    let resolvedSemesterName: String = {
                        ...
                        return semesterName
                            .isEmpty ? NSLocalizedString("courses.default.current_term", comment: "") : semesterName
                    }()
                    ...
                    let newCourse = CoursePageCourse(
                        ...
                        instructor: instructor
                            .isEmpty ? NSLocalizedString("courses.default.tbd", comment: "") : instructor,
                        ...
                        gradeInfo: course?.gradeInfo ?? CourseGradeInfo(
            ...
            VStack(alignment: .leading, spacing: ItariSpacing.m) {
                ...
                VStack(alignment: .leading, spacing: ItariSpacing.s) {
                    ...
                    HStack {
                        Text(String(
                            format: NSLocalizedString("courses.grade.percent_display", comment: ""),
                            Int(gradePercentInput)
                        ))
                        Spacer()
                        TextField(
                            NSLocalizedString("courses.grade.letter_placeholder", comment: ""),
                            text: $gradeLetterInput
                        )
                        .textFieldStyle(.roundedBorder)
                        .frame(width: 80)
                    }
                ...
                HStack {
                    ...
                    Button(NSLocalizedString("courses.form.button.save", comment: "")) {
                        if let courseId = currentSelection?.id {
                            gradesStore.upsert(
                                courseId: courseId,
                                percent: gradePercentInput,
                                letter: gradeLetterInput.isEmpty ? nil : gradeLetterInput
                            )
                        }
                        showingGradeSheet = false
                    }
