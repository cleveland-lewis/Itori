    ...
    struct DashboardView: View {
            ...
            while weekStart <= semesterEnd {
                ...
                let label = String(
                    format: NSLocalizedString(
                        "dashboard.grades.week_label",
                        value: "W%d",
                        comment: "Label for week on the grades graph"
                    ),
                    weekIndex
                )
        ...
        private func semesterTasks(_ semester: Semester, upTo cutoff: Date? = nil) -> [AppTask] {
            let courseIDs = Set(semesterCourses(semester).map(\.id))
        ...
        private enum GradeWidgetMode: String, CaseIterable, Identifiable {
            ...
            var title: String {
                ...
                case .currentSemester:
                    NSLocalizedString(
                        "dashboard.grades.mode.current",
                        value: "Current semester",
                        comment: "Current semester filter label"
                    )
                case .allSemesters:
                    NSLocalizedString(
                        "dashboard.grades.mode.all",
                        value: "All semesters",
                        comment: "All semesters filter label"
                    )
                }
            }
        }
        ...
        private var statusHeadline: String {
            ...
            let plannedCount = assignmentsStore.tasks
                .filter {
                    !$0.isCompleted && $0.type != .practiceTest && AssignmentPlansStore.shared.plan(for: $0.id) != nil
                }
                .count
            ...
            DashboardCard(
                ...
                HStack(spacing: 32) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Text(
                            verbatim: "\(assignmentsStore.tasks.filter { !$0.isCompleted && $0.type != .practiceTest }.count)"
                        )
                        .font(.system(size: statNumberSize, weight: .bold))
                        .contentTransition(.numericText())
                    }
            ...
            DashboardCard(
                ...
                if events.count > 5 {
                    Button {
                        ...
                        HStack {
                            Text(String(
                                format: NSLocalizedString("dashboard.calendar.view_all_events", comment: ""),
                                events.count
                            ))
                            Spacer()
        ...
        private var assignmentsCard: some View {
            DashboardCard(
                ...
                Button {
                    ...
                    Label(
                        NSLocalizedString("Add Assignment", value: "Add Assignment", comment: ""),
                        systemImage: "plus"
                    )
                ...
                if total > 6 {
                    Button {
                        ...
                        dashboardButtonLabel(
                            title: NSLocalizedString("dashboard.button.view_all", comment: ""),
        ...
        private var weeklyWorkloadChart: some View {
            ...
            .chartForegroundStyleScale(
                domain: TaskType.allCases.map(\.displayName),
        ...
        private func courseTitle(for courseId: UUID?) -> String {
            guard let courseId,
                  let course = coursesStore.activeCourses.first(where: { $0.id == courseId })
            else {
                return ""
            }
            return course.title
            ...
            .contextMenu {
                if !task.isDone {
                    Button {
                        ...
                        Label(
                            NSLocalizedString("Mark Complete", value: "Mark Complete", comment: ""),
                ...
                Button {
                    ...
                    Label(
                        NSLocalizedString("Edit Assignment", value: "Edit Assignment", comment: ""),
                        systemImage: "pencil"
                    )
                }
                    ...
                    if selectedGradePoints.isEmpty {
                        DashboardEmptyState(
                            title: NSLocalizedString(
                                "dashboard.grades.empty.title",
                                value: "No grade data yet",
                                comment: ""
                            ),
                            systemImage: "chart.xyaxis.line",
                            description: NSLocalizedString(
                                "dashboard.grades.empty.description",
                                value: "Add some graded tasks or link your courses to see GPA trends.",
                                comment: ""
                            )
                        )
                        .frame(maxWidth: .infinity)
        ...
        private var selectedGradePoints: [GradeWidgetPoint] {
            ...
            case .currentSemester:
                currentSemesterGradePoints
            case .allSemesters:
                allSemestersGradePoints
            }
        }
        ...
        private var gradeSummaryText: String? {
            ...
            case .currentSemester:
                return String(
                    format: NSLocalizedString(
                        "dashboard.grades.current_summary",
                        value: "Current GPA: %.2f",
                        comment: "Summary text for the current semester GPA"
                    ),
                    coursesStore.currentGPA
                )
            case .allSemesters:
                guard let average = selectedGradeAverage else { return nil }
                return String(
                    format: NSLocalizedString(
                        "dashboard.grades.all_summary",
                        value: "Avg GPA: %.2f",
                        comment: "Summary text for the all semesters GPA"
                    ),
                    average
                )
            }
        ...
        private func currentSemesterGradeChart(points: [GradeWidgetPoint]) -> some View {
            ...
            .chartXAxisLabel(NSLocalizedString(
                "dashboard.chart.axis.week",
                value: "Week",
                comment: "XAxis label for current semester grade chart"
            ))
            .chartYAxisLabel(NSLocalizedString(
                "dashboard.chart.axis.gpa",
                value: "GPA",
                comment: "YAxis label for grade charts"
            ))
        }
        ...
        private func allSemestersGradeChart(points: [GradeWidgetPoint]) -> some View {
            ...
            .chartXAxisLabel(NSLocalizedString(
                "dashboard.chart.axis.semester",
                value: "Semester",
                comment: "XAxis label for semester GPA chart"
            ))
            .chartYAxisLabel(NSLocalizedString(
                "dashboard.chart.axis.gpa",
                value: "GPA",
            ...
            DashboardCard(
                ...
                if snapshot.plannedMinutes <= 0 {
                    DashboardEmptyState(
                        ...
                        description: NSLocalizedString(
                            "dashboard.planner.no_plan_time.description",
                            value: "Once your planner is scheduled, progress shows here.",
                            comment: ""
                        )
                    )
        ...
        private var calendarCard: some View {
            DashboardCard(
                ...
                Button {
                    ...
                    dashboardButtonLabel(
                        title: NSLocalizedString("dashboard.calendar.open_calendar", comment: ""),
                        systemImage: "arrow.right",
                        trailingIcon: true
                    )
                    .font(.subheadline)
        ...
        private func todaysCalendarEvents() -> [EKEvent] {
            ...
            return source.filter { event in
                cal.isDateInToday(event.startDate) &&
                    (calendarManager.selectedCalendarID.isEmpty || event.calendar.calendarIdentifier == calendarManager
                        .selectedCalendarID)
            }
        }
    ...
    private struct DashboardCalendarColumn: View {
        ...
        private var dayItems: [DayItem] {
            days.map { DayItem(
                date: $0,
                isCurrentMonth: calendar.isDate($0, equalTo: selectedDate, toGranularity: .month)
            ) }
        }
    ...
    private struct DashboardTasksColumn: View {
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: DesignSystem.Spacing.medium) {
                Text(NSLocalizedString(
                    "dashboard.assignments.due_today",
