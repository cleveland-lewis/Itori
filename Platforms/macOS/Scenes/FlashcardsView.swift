    ...
    struct FlashcardsView: View {
        ...
        private var currentSemesterCourses: [Course] {
            let existingDeckCourseIds = Set(manager.decks.compactMap(\.courseID))
                    ...
                    VStack(spacing: 12) {
                        HStack {
                            ...
                            Button {
                                ...
                                Label(
                                    NSLocalizedString("flashcards.action.create_deck", comment: "Create new deck"),
                                    systemImage: "plus"
                                )
                            }
                        ...
                        TextField(
                            NSLocalizedString("flashcards.search.placeholder", comment: "Search decks"),
                            text: $searchText
                        )
        ...
        private var emptyStateView: some View {
            VStack(spacing: 16) {
                ...
                Text(searchText
                    .isEmpty ? NSLocalizedString("flashcards.empty.no_decks", comment: "No Decks") : NSLocalizedString(
                        "flashcards.empty.no_results",
                        comment: "No Results"
                    ))
                    .font(.headline)
                ...
                if searchText.isEmpty {
                    Text(NSLocalizedString(
                        "flashcards.empty.create_first",
                        comment: "Create your first flashcard deck to get started"
                    ))
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
        ...
        private var emptyDetailView: some View {
            VStack(spacing: 16) {
                ...
                Text(NSLocalizedString(
                    "flashcards.empty.choose_deck",
                    comment: "Choose a deck from the sidebar to view cards and study"
                ))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
    ...
    struct DeckCardView: View {
        ...
        private var dueCount: Int {
            deck.cards.filter(\.isDue).count
        }

        private var newCount: Int {
            deck.cards.filter(\.isNew).count
        }
    ...
    struct CourseRowView: View {
        ...
        var body: some View {
            ...
            .buttonStyle(.plain)
            .help(String(
                format: NSLocalizedString("flashcards.deck.help_create", comment: "Create flashcard deck for %@"),
                course.title
            ))
        }
                ...
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        HStack(spacing: 8) {
                            ...
                            Text(
                                "\(deck.cards.count) \(NSLocalizedString("flashcards.deck.cards_count", comment: "%d cards"))"
                            )
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        }
