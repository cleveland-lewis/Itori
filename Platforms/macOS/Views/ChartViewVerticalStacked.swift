            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack {
                    Text(NSLocalizedString(
                        "chartverticalstacked.categories",
                        value: "Categories",
                        comment: "Categories"
                    ))
                ...
                if isLegendExpanded {
                    ...
                    let columns = [GridItem(
                        ...
                        spacing: DesignSystem.Layout.spacing.small
                    )]
                    ...
                    ForEach(buckets) { bucket in
                        VStack(spacing: 6) {
                            ZStack(alignment: .bottom) {
                                let keys = bucket.categoryDurations.keys.sorted()
                                let segments: [(key: String, start: Double, value: Double)] = keys.reduce(into: (
                                    acc: 0.0,
                                ...
                                ForEach(segments, id: \.key) { segment in
                                    let isHighlight = hoveredPoint == nil ||
                                        (hoveredPoint?.bucket == bucket.label && hoveredPoint?.category == segment.key)
