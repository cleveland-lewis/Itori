    ...
    struct AddExamPopup: View {
            ...
            ItoriPopupContainer(title: "New Exam", subtitle: "Add exam details and study materials") {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Picker("Course", selection: Binding(get: { selectedCourseId }, set: { selectedCourseId = $0 })) {
                        Text(NSLocalizedString("ui.select.course", value: "Select course", comment: "Select course"))
                            .tag(UUID?(nil))
                    ...
                    Section(header: Text(NSLocalizedString(
                        ...
                        comment: "Study Materials"
                    ))) {
                        Button(action: { showImporter = true }) {
                            HStack { Image(systemName: "tray.and.arrow.up")
                                Text(NSLocalizedString(
                                    "ui.upload.syllabus.practice.test",
                                    value: "Upload Syllabus / Practice Test",
                                    comment: "Upload Syllabus / Practice Test"
                                ))
                            }
                        }
                        ...
                        Toggle(
                            NSLocalizedString(
                                "ui.toggle.generate.study.guide",
                                value: "Generate Study Guide",
                                comment: "Generate Study Guide"
                            ),
                            isOn: $generateStudyGuide
                        )
                        .disabled(uploadedURLs.isEmpty)

                        Toggle(
                            NSLocalizedString(
                                "ui.toggle.create.flashcard.deck",
                                value: "Create Flashcard Deck",
                                comment: "Create Flashcard Deck"
                            ),
                            isOn: $createFlashcardDeck
                        )
                        .disabled(uploadedURLs.isEmpty)
                    }
        ...
        private func saveExam() {
            ...
            // Create a placeholder exam entry in CoursesStore (not defined here) — for now just create deck if
            // requested
