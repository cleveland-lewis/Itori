    ...
    struct IOSGradesView: View {
                ...
                if coursesStore.activeCourses.isEmpty {
                    IOSInlineEmptyState(
                        title: NSLocalizedString(
                            "ios.grades.empty.title",
                            value: "No courses yet",
                            comment: "No courses"
                        ),
                        subtitle: NSLocalizedString(
                            "ios.grades.empty.subtitle",
                            value: "Add courses to track grades",
                            comment: "Add courses to track grades"
                        )
                    )
                } else {
                    Section {
        ...
        private var overallGPACard: some View {
            VStack(spacing: 12) {
                ...
                if !coursesStore.activeCourses.isEmpty {
                    ...
                    HStack {
                        statItem(
                            label: NSLocalizedString("ios.grades.courses", value: "Courses", comment: "Courses"),
                            value: "\(coursesStore.activeCourses.count)"
                        )
                        Spacer()
                        statItem(
                            label: NSLocalizedString("ios.grades.graded", value: "Graded", comment: "Graded"),
                            value: "\(gradedCoursesCount)"
                        )
                    }
                }
            }
            ...
            .accessibilityLabel(
                "Overall GPA: \(String(format: "%.2f", calculateOverallGPA())), \(coursesStore.activeCourses.count) courses, \(gradedCoursesCount) graded"
            )
        }
        ...
        private func calculateOverallGPA() -> Double {
            let gradedCourses = coursesStore.activeCourses.compactMap { course -> (course: Course, percent: Double)? in
                guard let gradeEntry = gradesStore.grade(for: course.id),
                      let percent = gradeEntry.percent
                else {
                    return nil
                }
        ...
        private func gradeIcon(_ percent: Double) -> String {
            switch percent {
            case 90 ... 100: "star.fill"
            case 80 ..< 90: "hand.thumbsup.fill"
            case 70 ..< 80: "minus.circle.fill"
        ...
        private func gpaIcon(_ gpa: Double) -> String {
            switch gpa {
            case 3.5 ... 4.0: "star.fill"
            case 3.0 ..< 3.5: "hand.thumbsup.fill"
            case 2.0 ..< 3.0: "minus.circle.fill"
            default: "exclamationmark.triangle.fill"
            }
        }
            ...
            NavigationStack {
                List {
                    Section(NSLocalizedString(
                        "ios.grades.overall_grade",
                        value: "Overall Grade",
                        comment: "Overall Grade"
                    )) {
                        if isEditing {
                            HStack {
                                TextField(
                                    NSLocalizedString("ios.grades.percent", value: "Percent", comment: "Percent"),
                                    text: $editingPercent
                                )
                            ...
                            TextField(
                                NSLocalizedString("ios.grades.letter", value: "Letter Grade", comment: "Letter Grade"),
                                text: $editingLetter
                            )
                        } else {
                            if let gradeEntry = gradesStore.grade(for: course.id) {
                                ...
                                Text(NSLocalizedString(
                                    "ios.grades.no_grade_entered",
                                    value: "No grade entered",
                                    comment: "No grade entered"
                                ))
                                .foregroundStyle(.secondary)
                            }
                        }
                ...
                .toolbar {
                    ...
                    ToolbarItem(placement: .topBarTrailing) {
                        Button(isEditing ? NSLocalizedString("ios.grades.save", value: "Save", comment: "Save") :
                            NSLocalizedString(
                                "ios.grades.edit",
                                value: "Edit",
                                comment: "Edit"
