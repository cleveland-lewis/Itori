    ...
    struct AutoRescheduleHistoryView: View {
            ...
            .toolbar {
                if !engine.rescheduleHistory.isEmpty {
                    Button(NSLocalizedString(
                        "settings.planner.reschedule.history.clear",
                        value: "Clear",
                    ...
                    .confirmationDialog(
                        NSLocalizedString(
                            "settings.planner.reschedule.history.clear.title",
                            value: "Clear Reschedule History",
                            comment: "Clear history title"
                        ),
                        ...
                        Button(
                            NSLocalizedString(
                                "settings.planner.reschedule.history.clear.action",
                                value: "Clear History",
                                comment: "Clear history action"
                            ),
                            role: .destructive
                        ) {
                            clearHistory()
                        ...
                        Text(NSLocalizedString(
                            "settings.planner.reschedule.history.clear.message",
                            value: "This will permanently delete all reschedule history. This cannot be undone.",
                            comment: "Clear history message"
                        ))
                    }
                }
            }
        ...
        private var emptyState: some View {
            VStack(spacing: 16) {
                ...
                Text(NSLocalizedString(
                    "settings.planner.reschedule.history.empty.title",
                    value: "No Reschedule History",
                    comment: "Reschedule history empty title"
                ))
                .font(.title3)
                .fontWeight(.semibold)

                Text(NSLocalizedString(
                    "settings.planner.reschedule.history.empty.body",
                    value: "When tasks are automatically rescheduled, they'll appear here",
                    comment: "Reschedule history empty body"
                ))
                .font(.subheadline)
                .foregroundColor(.secondary)
    ...
    struct HistoryRow: View {
                    ...
                    HStack(spacing: 4) {
                        ...
                        Text(String(
                            format: NSLocalizedString(
                                "settings.planner.reschedule.history.pushed",
                                value: "Pushed %d task(s)",
                                comment: "Pushed tasks count"
                            ),
                            operation.pushedSessions.count
                        ))
                        .font(.caption)
                    }
        ...
        private var strategyIcon: String {
            ...
            case .sameDaySlot: "checkmark.circle.fill"
            case .sameDayPushed: "arrow.up.circle.fill"
            case .nextDay: "calendar.circle.fill"
            case .overflow: "exclamationmark.triangle.fill"
            }
        }

        private var strategyColor: Color {
            switch operation.strategy {
            case .sameDaySlot: Color.Status.success // High-contrast green (4.8:1)
            case .sameDayPushed: Color.Status.warning // High-contrast orange (4.6:1)
            case .nextDay: Color.Status.info // High-contrast blue (5.5:1)
            case .overflow: Color.Status.error // High-contrast red (5.1:1)
            }
        }
