            ...
            List {
                Section {
                    ...
                    HStack {
                        ...
                        Spacer()
                        Text("\(settings.pomodoroShortBreakStorage) " + NSLocalizedString(
                            "time.minutes",
                            comment: "min"
                        ))
                        .foregroundColor(.secondary)
                    ...
                    HStack {
                        ...
                        Spacer()
                        Text("\(settings.pomodoroLongBreakStorage) " + NSLocalizedString(
                            "time.minutes",
                            comment: "min"
                        ))
                        .foregroundColor(.secondary)
                ...
                Section {
                    Picker(
                        NSLocalizedString("settings.timer.display", comment: "Timer display"),
                        selection: timerDisplayStyleBinding
                    ) {
                ...
                Section {
                    Toggle(isOn: $settings.timerAlertsEnabledStorage) {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text(NSLocalizedString(
                                "settings.timer.alerts.detail",
                                comment: "Show notification when timer completes"
                            ))
                            .font(.caption)
                            .foregroundColor(.secondary)
                        }
                    ...
                    Toggle(isOn: $settings.pomodoroAlertsEnabledStorage) {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text(NSLocalizedString(
                                "settings.timer.pomodoro_alerts.detail",
                                comment: "Alert at each pomodoro phase change"
                            ))
                            .font(.caption)
                            .foregroundColor(.secondary)
                        }
