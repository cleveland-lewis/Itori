            ...
            NavigationStack {
                ...
                .toolbar {
                    ...
                    ToolbarItem(placement: .topBarTrailing) {
                        Button(NSLocalizedString(
                            "iospracticetesttaking.button.submit",
                            value: "Submit",
                            comment: "Submit"
                        )) {
                ...
                .alert("Submit Test?", isPresented: $showingSubmitConfirmation) {
                    ...
                    if userAnswers.count < test.questions.count {
                        Text(
                            verbatim: "You have answered \(userAnswers.count) out of \(test.questions.count) questions. Unanswered questions will be marked incorrect."
                        )
                    } else {
                        Text(NSLocalizedString(
                            "iospracticetesttaking.are.you.sure.you.want",
                            value: "Are you sure you want to submit? You cannot change your answers after submission.",
                            comment: "Are you sure you want to submit? You cannot change..."
                        ))
                    }
                }
            }
            ...
            HStack(spacing: 12) {
                if currentQuestionIndex > 0 {
                    Button {
                        ...
                        Label(
                            NSLocalizedString(
                                "iospracticetesttaking.label.previous",
                                value: "Previous",
                                comment: "Previous"
                            ),
                ...
                if currentQuestionIndex < test.questions.count - 1 {
                    Button {
                        ...
                        Label(
                            NSLocalizedString("iospracticetesttaking.label.next", value: "Next", comment: "Next"),
        ...
        private var emptyState: some View {
            VStack(spacing: 12) {
                Image(systemName: "questionmark.circle")
                    ...
                    .accessibilityHidden(true)
                Text(NSLocalizedString(
                    "iospracticetesttaking.no.questions.available",
                    value: "No questions available",
                    comment: "No questions available"
                ))
                .font(.title3.bold())
                Text(NSLocalizedString(
                    "iospracticetesttaking.something.went.wrong.loading.the.test",
                    value: "Something went wrong loading the test",
                    comment: "Something went wrong loading the test"
                ))
                .font(.subheadline)
                .foregroundStyle(.secondary)
            }
