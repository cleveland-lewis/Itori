            ...
            VStack(spacing: 24) {
                ...
                VStack(spacing: 12) {
                    Text(NSLocalizedString(
                        "iosflashcards.no.flashcard.decks",
                        value: "No Flashcard Decks",
                        comment: "No Flashcard Decks"
                    ))
                    .font(.title2.weight(.bold))

                    Text(NSLocalizedString(
                        "iosflashcards.create.your.first.deck.to",
                        value: "Create your first deck to start studying with spaced repetition",
                        comment: "Create your first deck to start studying with spac..."
                    ))
                    .font(.body)
                    .foregroundStyle(.secondary)
                ...
                Button {
                    ...
                    Label(
                        NSLocalizedString(
                            "iosflashcards.label.create.deck",
                            value: "Create Deck",
                            comment: "Create Deck"
                        ),
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack(spacing: 16) {
                    Label(
                        NSLocalizedString(
                            "iosflashcards.label.deckcardscount",
                            value: "\(deck.cards.count)",
                            comment: "\(deck.cards.count)"
                        ),
                    ...
                    if dueCount > 0 {
                        Label(
                            NSLocalizedString("iosflashcards.label.due.now", value: "Due now", comment: "Due now"),
                            systemImage: "clock"
                        )
                        .font(.caption)
                    ...
                    if !dueCards.isEmpty {
                        ...
                        HStack {
                            Image(systemName: "checkmark.circle.fill")
                                ...
                                .accessibilityHidden(true)
                            Text(NSLocalizedString(
                                "iosflashcards.all.caught.up",
                                value: "All caught up!",
                                comment: "All caught up!"
                            ))
                            .foregroundStyle(.secondary)
                        }
                    }
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 12) {
                    if isDue {
                        Label(
                            NSLocalizedString("iosflashcards.label.due", value: "Due", comment: "Due"),
                            systemImage: "clock"
                        )
                        .font(.caption)
                        .foregroundStyle(.orange)
            ...
            VStack(spacing: 24) {
                ...
                VStack(spacing: 12) {
                    HStack(spacing: 12) {
                        Label(
                            NSLocalizedString(
                                "iosflashcards.label.difficulty",
                                value: "Difficulty",
                                comment: "Difficulty"
                            ),
                            systemImage: "gauge"
                    ...
                    if let lastReviewed = card.lastReviewed {
                        HStack(spacing: 12) {
                            Label(
                                NSLocalizedString(
                                    "iosflashcards.label.last.reviewed",
                                    value: "Last Reviewed",
                                    comment: "Last Reviewed"
                                ),
                                systemImage: "clock"
                            )
                            .font(.caption)
    ...
    struct IOSStudySessionView: View {
            ...
            NavigationStack {
                ZStack {
                    if sessionComplete {
                        ...
                        Text(NSLocalizedString(
                            "iosflashcards.no.cards.to.study",
                            value: "No cards to study",
                            comment: "No cards to study"
                        ))
                    }
                }
            ...
            VStack(spacing: 24) {
                ...
                if !showingBack {
                    Button {
                        ...
                        Label(
                            NSLocalizedString(
                                "iosflashcards.label.show.answer",
                                value: "Show Answer",
                                comment: "Show Answer"
                            ),
                            systemImage: "eye"
        ...
        private func ratingButtons(card: Flashcard) -> some View {
            VStack(spacing: 12) {
                Text(NSLocalizedString(
                    "iosflashcards.how.well.did.you.know.this",
                    value: "How well did you know this?",
                    comment: "How well did you know this?"
                ))
        ...
        private func ratingButton(
            title: String,
            color: Color,
            rating: FlashcardManager.FlashcardRating,
            card: Flashcard
        ) -> some View {
            Button {
        ...
        private var completionView: some View {
            VStack(spacing: 24) {
                ...
                VStack(spacing: 8) {
                    Text(NSLocalizedString(
                        "iosflashcards.session.complete",
                        value: "Session Complete!",
                        comment: "Session Complete!"
                    ))
                    .font(.title.bold())
