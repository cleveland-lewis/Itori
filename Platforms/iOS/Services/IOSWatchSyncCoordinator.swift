...
//
    ...
    final class IOSWatchSyncCoordinator: NSObject, ObservableObject {
        ...
        private func startPeriodicSync() {
            ...
            IOSWatchSyncCoordinator
                .log("🔄 IOSWatchSyncCoordinator: Periodic sync enabled (every \(Int(syncInterval))s)")
        }
        ...
        func syncToWatch() {
            ...
            do {
                ...
                if session.isReachable {
                    session.sendMessage(["snapshot": snapshotData], replyHandler: { _ in
                        IOSWatchSyncCoordinator.log("✅ IOSWatchSyncCoordinator: Fallback message sent")
                    }, errorHandler: { error in
                        IOSWatchSyncCoordinator
                            .log("❌ IOSWatchSyncCoordinator: Fallback message failed: \(error.localizedDescription)")
                    })
                }
            }
        }
        ...
        private func createSnapshot() -> WatchSnapshot {
            // Get active timer
            let activeTimer: ActiveTimerSummary? = if let timerManager, timerManager.isRunning {
                ActiveTimerSummary(
                    id: UUID(),
        ...
        private func handleWatchMessage(_ message: [String: Any]) -> [String: Any] {
            IOSWatchSyncCoordinator
                .log("📥 IOSWatchSyncCoordinator: Handling message: \(message.keys.joined(separator: ", "))")
        ...
        private func handleToggleTask(_ message: [String: Any]) {
            guard let store = assignmentsStore,
                  ...
                  let taskIndex = store.tasks.firstIndex(where: { $0.id == taskId })
            else {
                return
            }
        ...
        private func handleAddTask(_ message: [String: Any]) {
            ...
            let dueDate: Date? = if let dueISO = message["dueISO"] as? String {
                ISO8601DateFormatter().date(from: dueISO)
            } else {
                nil
            }
    ...
    extension IOSWatchSyncCoordinator: WCSessionDelegate {
        ...
        nonisolated func session(
            ...
            Task { @MainActor in
                ...
                if let error {
                    IOSWatchSyncCoordinator
                        .log("❌ IOSWatchSyncCoordinator: Activation error: \(error.localizedDescription)")
                } else {
