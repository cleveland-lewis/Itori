...
//
    ...
    struct IOSTimerPageView: View {
        ...
        var body: some View {
            if isIPad {
                iPadLayout
                    .modifier(TimerSyncModifiers(
                        viewModel: viewModel,
                        settings: settings,
                        syncLiveActivity: syncLiveActivity,
                        syncSettingsFromApp: syncSettingsFromApp
                    ))
            } else {
                mainScroll
                    .modifier(TimerSyncModifiers(
        ...
        private var pinnedActivities: [TimerActivity] {
            filteredActivities.filter(\.isPinned)
        }
        ...
        private func activityRow(_ activity: TimerActivity) -> some View {
            ...
            .contextMenu {
                ...
                Button {
                    ...
                    Label(
                        NSLocalizedString("timer.activity.select", comment: "Select"),
                ...
                if let activity = selectedActivity {
                    ...
                    VStack(spacing: 12) {
                        ...
                        Text(NSLocalizedString(
                            "Select an activity to add notes",
                            value: "Select an activity to add notes",
                            comment: ""
                        ))
                        .font(.subheadline)
                        .foregroundColor(.secondary)
    ...
    private struct TimerSyncModifiers: ViewModifier {
        ...
        func body(content: Content) -> some View {
            content
                .modifier(TimerSettingsSync(
                    settings: settings,
                    requestAlarmAuthorization: requestAlarmAuthorization,
                    syncSettingsFromApp: syncSettingsFromApp
                ))
