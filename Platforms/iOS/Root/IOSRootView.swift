    ...
    struct IOSRootView: View {
        ...
        var body: some View {
            bodyContent
                ...
                .alert(
                    NSLocalizedString(
                        "sync.conflict.courses.title",
                        value: "Courses Sync Conflict",
                        comment: "Courses sync conflict title"
                    ),
                    isPresented: $showingCoursesSyncConflict
                ) {
                    Button(NSLocalizedString(
                        "sync.conflict.courses.keep_local",
                        value: "Keep Local",
                        comment: "Keep local data"
                    )) {
                        CoursesStore.shared?.resolveSyncConflict(useCloud: false)
                    }
                    Button(
                        NSLocalizedString(
                            "sync.conflict.courses.use_icloud",
                            value: "Use iCloud",
                            comment: "Use iCloud data"
                        ),
                        role: .destructive
                    ) {
                    ...
                    Text(NSLocalizedString(
                        "sync.conflict.courses.message",
                        value: "Courses and semesters differ between this device and iCloud. Choose which data to keep.",
                        comment: "Courses sync conflict message"
                    ))
                }
        ...
        private var nonStarredTabs: [RootTab] {
            TabRegistry.allTabs.map(\.id).filter { !starredTabs.contains($0) }
        }
        ...
        private func sheetContent(for sheet: IOSSheetRouter.SheetKind) -> some View {
            switch sheet {
            case let .addAssignment(defaults):
                IOSTaskEditorView(
                    task: nil,
                    ...
                    onSave: { draft in
                        ...
                        toastRouter.show(String(
                            format: NSLocalizedString("ios.toast.assignment_added", comment: "Assignment added"),
                            defaults.itemLabel
                        ))
                    }
                )
            case let .addCourse(defaults):
                IOSCourseEditorView(
                    ...
                    onSave: { draft in
                        guard let semester = coursesStore.semesters.first(where: { $0.id == draft.semesterId })
                        else { return }
