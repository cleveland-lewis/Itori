...
struct CalendarPageView: View {
    ...
    private func updateMetrics() {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            let calculatedMetrics = CalendarStats.calculate(from: eventsToProcess, for: dateToProcess)
        ...
        GeometryReader { proxy in
            if calendar.isDateInToday(focusedDate) {
                ...
                .overlay(
                    HStack(spacing: 4) {
                        Text("•")
                        Text(LocaleFormatters.timeFormatter(use24Hour: AppSettingsModel.shared.use24HourTime).string(from: Date()))
                            .font(.caption2)
                    }
...
private struct AllDayRow: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            ForEach(days, id: \.self) { day in
                VStack(alignment: .leading, spacing: 2) {
                    ForEach(eventsForDay(day).prefix(2)) { event in
                        Text(event.title)
                            .font(.caption2)
                            .lineLimit(1)
...
private struct CalendarDayTimelineView: View {
    ...
    private var dayEvents: [CalendarEvent] {
        events
            .filter { calendar.isDate($0.startDate, inSameDayAs: date) && !isAllDayEvent($0) }
            .sorted { $0.startDate < $1.startDate }
    }
