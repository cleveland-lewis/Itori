            ...
            if let event = selectedEvent {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text(timeFormatter(event.startDate, event.endDate))
                        .font(DesignSystem.Typography.body)
                    if let location = event.location, !location.isEmpty {
                        Text(String(format: NSLocalizedString("calendar.location_label", comment: "Location field label"), location))
                            .font(DesignSystem.Typography.body)
                    }
                            ...
                            if !dayEvents.isEmpty {
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    if dayEvents.count > 3 {
                                        Text(String(format: NSLocalizedString("calendar.more_events", comment: "More events indicator"), dayEvents.count - 3))
                                            .font(DesignSystem.Typography.caption)
                                            .foregroundStyle(.secondary)
                                    }
                                }
        ...
        VStack(alignment: .leading, spacing: 16) {
                ...
                if let travelTime = item.travelTime, travelTime > 0 {
                    HStack(spacing: DesignSystem.Layout.spacing.small) {
                        Image(systemName: "car.fill")
                            ...
                            .symbolRenderingMode(.hierarchical)
                            .frame(width: 24)
                        Text(String(format: NSLocalizedString("calendar.travel_time", comment: "Travel time label"), CalendarManager.TravelTimeOption.from(interval: travelTime).rawValue))
                            .font(.body)
                            .foregroundStyle(.primary)
                    }
                }
            ...
            if let identifier = item.ekIdentifier {
                ...
                .confirmationDialog("Delete this item?", isPresented: $showDeleteConfirm, titleVisibility: .visible) {
                    ...
                    Text(String(format: NSLocalizedString("calendar.delete_confirmation", comment: "Delete confirmation message"), item.isReminder ? NSLocalizedString("calendar.reminders", comment: "Reminders") : NSLocalizedString("calendar.calendar", comment: "Calendar")))
                }
            }
        }
