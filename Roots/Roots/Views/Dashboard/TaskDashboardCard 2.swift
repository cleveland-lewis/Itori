...
struct LegacyTaskDashboardCard: View {
    ...
    var completedTasks: [Task] {
        tasks.filter { $0.isCompleted }
            .sorted { $0.completedDate ?? Date() > $1.completedDate ?? Date() } // Newest first
    }
    
    // Sorting Logic: High Priority -> Earlier Due Date
    func sortTasks(_ t1: Task, _ t2: Task) -> Bool {
        if t1.priority != t2.priority {
            return t1.priority.rawValue > t2.priority.rawValue // High (2) before Low (0)
        }
        ...
        RootsCard {
            VStack(alignment: .leading, spacing: DesignSystem.Spacing.medium) {
                ...
                HStack {
                    ...
                    Spacer()
                NavigationLink(destination: Text("Full Task View")) { // Replace with destination
                ...
                if tasks.filter({ !$0.isCompleted }).isEmpty {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        if !overdueTasks.isEmpty {
                            TaskGroupView(tasks: $tasks, title: "Overdue", filteredTasks: overdueTasks, isUrgent: true)
                        }
                        
                        if !todayTasks.isEmpty {
                            TaskGroupView(tasks: $tasks, title: "Today", filteredTasks: todayTasks)
                        }
                        
                        if !tomorrowTasks.isEmpty {
                            TaskGroupView(tasks: $tasks, title: "Tomorrow", filteredTasks: tomorrowTasks)
                        }
                        
                        if !upcomingTasks.isEmpty {
                            TaskGroupView(tasks: $tasks, title: "Upcoming", filteredTasks: upcomingTasks)
                        }
                    }
                }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: DesignSystem.Layout.spacing.small) {
            ...
            ForEach(filteredTasks) { task in
                LegacyDashboardTaskRow(tasks: $tasks, task: task)
            }
        }
    }
}

struct LegacyDashboardTaskRow: View {
    @Binding var tasks: [Task]
    let task: Task
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                // Optional: Show Course Name or Time if needed
        ...
        .background(Color(nsColor: .controlBackgroundColor).opacity(0.5)) // Subtle row background
