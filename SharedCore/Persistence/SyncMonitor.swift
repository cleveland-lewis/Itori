...
import Combine
    ...
    final class SyncMonitor: ObservableObject {
        ...
        struct SyncEvent: Identifiable {
            ...
            var icon: String {
                switch type {
                case .importStarted, .importFinished: "arrow.down.circle"
                case .exportStarted, .exportFinished: "arrow.up.circle"
                case .conflict: "exclamationmark.triangle"
                case .error: "xmark.circle"
                case .statusChange: "info.circle"
                }
            }

            var color: String {
                switch type {
                case .importStarted, .exportStarted: "blue"
                case .importFinished, .exportFinished: "green"
                case .conflict: "orange"
                case .error: "red"
                case .statusChange: "gray"
                }
            }
        }
        ...
        private func setupObservers() {
            ...
            NotificationCenter.default
                .publisher(for: NSNotification.Name(rawValue: "NSPersistentCloudKitContainerEventChangedNotification"))
                .sink { [weak self] notification in
        ...
        private func handleRemoteChange(_: Notification) {
            Task { @MainActor in
        ...
        private func handleSyncStatusChange(_ notification: Notification) {
            Task { @MainActor in
                if let enabled = notification.object as? Bool {
                    ...
                    addEvent(
                        type: .statusChange,
                        details: "CloudKit sync \(enabled ? "enabled" : "disabled"): \(reason)"
                    )
                }
            }
        }
        ...
        private func checkCloudKitStatus() {
            Task { @MainActor in
                ...
                addEvent(
                    type: .statusChange,
                    details: "Initial status: CloudKit \(isCloudKitActive ? "active" : "inactive")"
                )
            }
        }
