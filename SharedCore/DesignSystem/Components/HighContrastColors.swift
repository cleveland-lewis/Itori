...
extension Color {
    ...
    static func accessible(_ color: Color, on _: Color = .white) -> Color {
        // For white backgrounds, use darker, more saturated versions
        if color == .blue {
            Color(red: 0.0, green: 0.3, blue: 0.8) // Darker blue: ~5.5:1
        } else if color == .red {
            Color(red: 0.7, green: 0.0, blue: 0.0) // Darker red: ~5.1:1
        } else if color == .green {
            Color(red: 0.0, green: 0.5, blue: 0.0) // Darker green: ~4.8:1
        } else if color == .orange {
            Color(red: 0.8, green: 0.35, blue: 0.0) // Darker orange: ~4.6:1
        } else if color == .purple {
            Color(red: 0.45, green: 0.0, blue: 0.7) // Darker purple: ~5.2:1
        } else if color == .yellow {
            Color(red: 0.7, green: 0.6, blue: 0.0) // Darker yellow: ~6.0:1
        } else if color == .pink {
            Color(red: 0.8, green: 0.0, blue: 0.3) // Darker pink: ~4.8:1
        } else {
            color // Use original for other colors
        }
    }
}
...
public extension View {
    ...
    @ViewBuilder
    func accessibleColor(_ color: Color, on background: Color = .white) -> some View {
        #if os(iOS)
