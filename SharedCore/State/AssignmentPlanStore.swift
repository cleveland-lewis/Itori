...
final class AssignmentPlanStore: ObservableObject {
    ...
    func toggleSequenceEnforcement(for assignmentId: UUID) {
        ...
        if plan.sequenceEnforcementEnabled && !plan.steps.contains(where: \.hasPrerequisites) {
        ...
        LOG_DATA(
            .info,
            "PlanStore",
            "Toggled sequence enforcement to \(plan.sequenceEnforcementEnabled) for assignment \(assignmentId)"
        )
    }
