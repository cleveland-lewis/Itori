...
final class CoursesStore: ObservableObject {
    weak static var shared: CoursesStore?
    ...
    init(storageURL: URL? = nil) {
        ...
        if let storageURL {
            ...
            let folder = dir.appendingPathComponent("ItoriCourses", isDirectory: true)
            try? fm.createDirectory(at: folder, withIntermediateDirectories: true)
            self.storageURL = folder.appendingPathComponent("courses.json")
            let cacheFolder = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        ...
        let snapshot = await Task.detached(priority: .userInitiated) { [weak self] () -> InitialCoursesSnapshot? in
            ...
            if FileManager.default.fileExists(atPath: self.cacheURL.path) {
                do {
                    ...
                    activeSemesterIds = decoded.activeSemesterIds
                        .map { Set($0) } ?? (currentSemesterId.map { [$0] } ?? [])
                    await LOG_PERSISTENCE(
                        .debug,
                        "CoursesCache",
                        "Loaded cache off-main",
                        metadata: ["semesters": "\(semesters.count)"]
                    )
                } catch {
    ...
    private func isDeleted(semesterId: UUID) -> Bool {
        semesters.first(where: { $0.id == semesterId })?.deletedAt != nil
    }
    ...
    func addCourse(title: String, code: String, to semester: Semester) {
        ...
        LOG_COURSES(
            ...
            "CourseAdded",
            "Course added: \(title)",
            metadata: ["courseId": newCourse.id.uuidString, "semesterId": semester.id.uuidString]
        )
        persist()
    ...
    var recentlyDeletedSemesters: [Semester] {
        semesters.compactMap { $0.deletedAt == nil ? nil : $0 }
            .sorted { ($0.deletedAt ?? Date.distantPast) > ($1.deletedAt ?? Date.distantPast) }
    }

    var futureSemesters: [Semester] {
        let now = Date()
        return semesters.filter { !$0.isArchived && $0.deletedAt == nil && $0.startDate > now }
        ...
        do {
            ...
            LOG_PERSISTENCE(
                ...
                metadata: [
                    ...
                    "activeSemesters": "\(activeSemesterIds.count)"
                ]
            )
        } catch {
    ...
    private func loadCache() {
        ...
        do {
            ...
            LOG_PERSISTENCE(
                ...
                "CoursesCache",
                "Loaded cache",
                metadata: ["semesters": "\(semesters.count)", "courses": "\(courses.count)"]
            )
        } catch {
...
extension CoursesStore {
    ...
    func moveNodeToParent(_ nodeId: UUID, newParentId: UUID?) -> Bool {
        ...
        node.sortIndex = siblings.isEmpty ? 0 : (siblings.map(\.sortIndex).max() ?? 0) + 1
        node.updatedAt = Date()
    ...
    func nextSortIndex(for parentId: UUID?) -> Int {
        ...
        return siblings.isEmpty ? 0 : (siblings.map(\.sortIndex).max() ?? 0) + 1
    }
}
...
extension CoursesStore {
    ...
    func updateFileCategory(fileId: UUID, category: FileCategory) {
        ...
        let highSignalCategories: [FileCategory] = [
            .syllabus,
            .test,
            .practiceTest,
            .rubric,
        ...
        DispatchQueue.global(qos: .utility).async {
            do {
                ...
                LOG_PERSISTENCE(
                    .info,
                    "CoursesiCloud",
                    "Synced courses to iCloud",
                    metadata: ["semesters": "\(snapshot.semesters.count)", "courses": "\(snapshot.courses.count)"]
                )
            } catch {
...
extension CoursesStore {
    ...
    private func hasSyncConflict(local: CoursesPersistedData, cloud: CoursesPersistedData) -> Bool {
        ...
        let localSemesterIds = Set(local.semesters.map(\.id))
        let cloudSemesterIds = Set(cloud.semesters.map(\.id))
        if localSemesterIds != cloudSemesterIds {
            return true
        }

        let localCourseIds = Set(local.courses.map(\.id))
        let cloudCourseIds = Set(cloud.courses.map(\.id))
    ...
    private func syncToCoreData() async {
        ...
        LOG_DATA(.debug, "CoursesSync", "CoreData sync not yet implemented for semesters/courses", metadata: [
            "semesters": "\(self.semesters.count)",
