...
public enum AISemanticValidationError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .dateOutOfRange(let date, let range):
            return "Date \(date) outside valid academic range \(range.lowerBound)...\(range.upperBound)"
        case .durationOutOfBounds(let dur, let range):
            return "Duration \(dur) outside valid range \(range.lowerBound)...\(range.upperBound)"
        case .emptyRequiredField(let field):
            return "Required field '\(field)' is empty"
        case .lowConfidence(let conf, let min):
            return "Confidence \(conf) below minimum \(min)"
        case .logicalInconsistency(let msg):
            return "Logical inconsistency: \(msg)"
        case .invalidRelationship(let msg):
            return "Invalid relationship: \(msg)"
        }
    }
}

/// Semantic validators for AI outputs
public struct AISemanticValidator {
    ...
    public static func validateDateOrdering(assigned: Date?, due: Date) throws {
        guard let assigned = assigned else { return }
...
public struct AIConfidencePolicy {
    ...
    public static func describe(confidence: Double) -> String {
        ...
        case 0.75..<0.9: return "Confident"
        case 0.5..<0.75: return "Moderate confidence"
        case 0.25..<0.5: return "Low confidence"
