...
public struct PDFTextExtractor {
    ...
    private func appearsMultiColumn(_ text: String) -> Bool {
        ...
        let avgLength = lines.map { $0.count }.reduce(0, +) / lines.count
    ...
    private func reorderMultiColumn(_ text: String) -> String {
        ...
        for line in lines {
            ...
            if mid > 0 {
                ...
                let right = String(line.suffix(from: line.index(line.startIndex, offsetBy: mid))).trimmingCharacters(in: .whitespaces)
...
private class HeaderFooterDetector {
    ...
    func removeRepeatingLines(from text: String) -> (result: String, removedLines: [String]) {
        ...
        let repeatingLines = lineCounts.filter { $0.value >= threshold }.map { $0.key }
...
public enum PDFExtractionError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .malformedStructure: return "PDF structure is malformed"
        }
    }
}
