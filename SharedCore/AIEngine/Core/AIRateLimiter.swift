...
public actor AIRateLimiter {
    ...
    public func allowRequest(for portID: String) -> Bool {
        ...
        if recentPortCount >= perPortRequestsPerMinute {
            logger
                .warning("Port rate limit exceeded for \(portID): \(recentPortCount)/\(self.perPortRequestsPerMinute)")
            return false
        }
    ...
    public func recordFailure(for portID: String) {
        ...
        if current + 1 >= maxConsecutiveFailures {
            ...
            logger
                .error(
                    "Circuit breaker opened for \(portID) after \(current + 1) failures, will retry at \(reopenTime)"
                )
        }
    }
