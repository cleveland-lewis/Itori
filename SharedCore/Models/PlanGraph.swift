...
public struct PlanGraph: Codable, Equatable, Sendable {
    ...
    public enum ValidationError: Error, Equatable, CustomStringConvertible {
        ...
        public var description: String {
            switch self {
            case let .cycleDetected(nodeIds):
                "Cycle detected in dependency graph: \(nodeIds.map { $0.uuidString.prefix(8) }.joined(separator: " → "))"
            case let .orphanEdge(edge):
                "Edge references non-existent node(s): \(edge.fromNodeId.uuidString.prefix(8)) → \(edge.toNodeId.uuidString.prefix(8))"
            case let .duplicateEdge(edge):
                "Duplicate edge: \(edge.fromNodeId.uuidString.prefix(8)) → \(edge.toNodeId.uuidString.prefix(8))"
            case let .duplicateNodeId(id):
                "Duplicate node ID: \(id.uuidString.prefix(8))"
            case let .selfLoop(id):
                "Self-loop detected: node \(id.uuidString.prefix(8)) depends on itself"
            case let .invalidNodeReference(id):
                "Invalid node reference: \(id.uuidString.prefix(8))"
            }
        }
    }
    ...
    public func getPrerequisites(for nodeId: UUID) -> [PlanNode] {
        let prereqIds = edges.filter { $0.toNodeId == nodeId }.map(\.fromNodeId)
    ...
    public func getDependents(for nodeId: UUID) -> [PlanNode] {
        let dependentIds = edges.filter { $0.fromNodeId == nodeId }.map(\.toNodeId)
    ...
    public func getRootNodes() -> [PlanNode] {
        let nodesWithPrereqs = Set(edges.map(\.toNodeId))
    ...
    public func getLeafNodes() -> [PlanNode] {
        let nodesWithDependents = Set(edges.map(\.fromNodeId))
...
public extension PlanGraph {
    /// Statistics about the graph
    struct GraphStatistics: Equatable {
    ...
    func getStatistics() -> GraphStatistics {
        GraphStatistics(
            ...
            completedNodes: nodes.filter(\.isCompleted).count,
