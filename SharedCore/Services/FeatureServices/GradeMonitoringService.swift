...
final class GradeMonitoringService: ObservableObject {
    ...
    private func detectGradeChange(for courseId: UUID) -> GradeAnalysis? {
        ...
        // Determine trend
        let trend: GradeTrend = if abs(change) < 2 {
            .stable
        } else if change > 0 {
            .improving
        } else {
    ...
    private func calculateStudyTimeIncrease(
        ...
        let increaseFactor = if gradeChange >= 15 {
            0.5 // 50% more time
    ...
    private func sendStudyTimeNotification(recommendation: StudyTimeRecommendation) async {
            ...
            do {
                ...
                LOG_NOTIFICATIONS(
                    .info,
                    "GradeMonitoring",
                    "Sent study time notification for \(recommendation.courseName)"
                )
            } catch {
