    ...
    final class EnhancedAutoRescheduleService: ObservableObject {
        ...
        private func getFreeTimeSlots(
            ...
            for day in 0 ..< daysAhead {
                ...
                guard let dayStart = calendar.date(from: dayStartComponents),
                      let dayEnd = calendar.date(from: dayEndComponents)
                else {
                    continue
                }
        ...
        private func sendRescheduleNotification(_ notification: RescheduleNotification) async {
            ...
            do {
                ...
                LOG_NOTIFICATIONS(
                    .info,
                    "EnhancedAutoReschedule",
                    "Sent reschedule notification for \(notification.assignmentTitle)"
                )
            } catch {
...
final class EnhancedAutoRescheduleService: ObservableObject {
    ...
    func clearNotification(id _: UUID) {
        // Disabled - needs API update
    }

    func checkForRescheduling(assignmentsStore _: AssignmentsStore, coursesStore _: CoursesStore?) async {
        // Disabled - needs API update
    }

    func acceptReschedule(_: RescheduleNotification, assignmentsStore _: AssignmentsStore) {
        // Disabled - needs API update
    }

    func dismissNotification(_: UUID) {
        // Disabled - needs API update
    }
