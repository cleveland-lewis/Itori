...
extension AssignmentPlanEngine {
    ...
    enum ValidationIssue: Equatable {
        ...
        var description: String {
            ...
            case .invalidSequenceIndex(let stepId, let expected, let actual):
                return "Step \(stepId) has invalid sequence index. Expected: \(expected), Actual: \(actual)"
            case .timeWindowOverlap(let step1, let step2):
                return "Time windows overlap between steps \(step1) and \(step2)"
            case .unrealisticDuration(let stepId, let duration):
                return "Step \(stepId) has unrealistic duration: \(duration) seconds"
            case .excessiveDuration(let stepId, let duration):
                return "Step \(stepId) has excessive duration: \(duration/3600) hours"
            case .dependencyCycle(let cycleIds):
                return "Dependency cycle detected: \(cycleIds)"
            }
        }
    }
}
...
extension AssignmentPlan {
    ...
    var qualityScore: Double {
        ...
        let durations = steps.map { $0.estimatedDuration }
    ...
    var complexityScore: Double {
        ...
        let uniqueTypes = Set(steps.map { $0.stepType }).count
