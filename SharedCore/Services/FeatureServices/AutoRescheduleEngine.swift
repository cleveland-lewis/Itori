...
final class AutoRescheduleEngine: ObservableObject {
    ...
    enum RescheduleStrategy: String, Codable {
        ...
        var displayName: String {
            switch self {
            case .sameDaySlot: "Rescheduled Today"
            case .sameDayPushed: "Rescheduled Today (Pushed Others)"
            case .nextDay: "Moved to Tomorrow"
            case .overflow: "Needs Manual Scheduling"
            }
        }
    }
    ...
    private func rescheduleSession(_ session: StoredScheduledSession) async -> RescheduleOperation? {
        ...
        if session.dueDate >= tomorrowStart, let slot = findFreeSlot(
            for: session,
            ...
            if conflicts.count <= maxPush {
                ...
                if canPushAll {
                    return (
                        ...
                        pushedSessionIds: conflicts.map(\.id)
                    )
                }
            }
    ...
    private func calculatePriority(_ session: StoredScheduledSession) -> Double {
        ...
        let categoryPriority = switch category {
        case .exam: 1.0
        case .quiz: 0.9
        case .project: 0.8
        case .homework: 0.7
