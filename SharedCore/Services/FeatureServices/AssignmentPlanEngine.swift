...
enum AssignmentPlanEngine {
    ...
    private static func generateExamSteps(
        ...
        for i in 0 ..< sessionCount {
            ...
            guard let recommendedStart = calendar.date(byAdding: .day, value: dayOffset, to: planStartDate)
            else { continue }

            let dueBy = (i == sessionCount - 1) ? dueDate : calendar.date(
                byAdding: .day,
                value: dayOffset + daysBetweenSessions - 1,
                to: planStartDate
            )
    ...
    private static func generateHomeworkSteps(
        ...
        if totalMinutes <= settings.homeworkSingleSessionThreshold {
            ...
            for i in 0 ..< sessionCount {
                ...
                guard let recommendedStart = calendar.date(byAdding: .day, value: dayOffset, to: startDate)
                else { continue }

                let dueBy = isLast ? dueDate : calendar.date(
                    byAdding: .day,
                    value: dayOffset + daysBetween,
                    to: startDate
                )
    ...
    private static func generatePracticeTestSteps(
        ...
        totalMinutes: Int,
        dueDate: Date,
        settings _: PlanGenerationSettings
    ) -> [PlanStep] {
        let planId = UUID()
        let step = PlanStep(
            planId: planId,
    ...
    private static func generateProjectSteps(
        ...
        for i in 0 ..< sessionCount {
            ...
            let stepType: PlanStep.StepType = switch i {
            case 0: .research
            case 1: .preparation
            case sessionCount - 1: .review
            default: .task
            }
    ...
    private static func convertExistingPlan(
        ...
        _ = oldPlan.reduce(0) { $0 + $1.expectedMinutes } // For future use
        let leadDays = settings.projectLeadDays
