...
class MockLLMBackend: LLMBackend {
    ...
    func generateJSON(prompt: String, schema _: String?) async throws -> String {
        try await Task.sleep(nanoseconds: UInt64(delay * 1_000_000_000))
    ...
    private func generateSlotJSON(from prompt: String) -> String {
        ...
        return """
        {
            ...
            "rationale": "This is the correct answer because it accurately reflects the principles of \(topic) at the \(
                bloomLevel
            ) cognitive level. The other options either misrepresent the concept or present incorrect applications.",
            "topic": "\(topic)",
            "bloomLevel": "\(bloomLevel)",
            "difficulty": "\(difficulty)",
            "templateType": "\(templateType)"
