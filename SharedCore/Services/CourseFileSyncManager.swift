...
final class CourseFileSyncManager: ObservableObject {
        ...
        for item in query.results {
            ...
            if let downloadStatus = metadataItem.value(forAttribute: NSMetadataUbiquitousItemDownloadingStatusKey) as? String {
                if downloadStatus == NSMetadataUbiquitousItemDownloadingStatusCurrent {
                    LOG_ICLOUD(.debug, "FileSyncManager", "File downloaded: \(metadataItem.value(forAttribute: NSMetadataItemDisplayNameKey) ?? "unknown")")
                }
            }
            
            // Check for conflicts
            if let hasConflict = metadataItem.value(forAttribute: NSMetadataUbiquitousItemHasUnresolvedConflictsKey) as? Bool,
               hasConflict {
    ...
    func uploadPendingFiles() async {
        ...
        do {
            let metadataFiles = try fileManager.contentsOfDirectory(at: pendingUploadsURL, includingPropertiesForKeys: nil)
            ...
            for metadataURL in metadataFiles {
                do {
                    ...
                    LOG_ICLOUD(.error, "FileSyncManager", "Failed to upload pending file: \(error.localizedDescription)")
                }
            }
