...
final class EventDeletionService {
    ...
    enum RecurringDeletionScope {
        ...
        var ekSpan: EKSpan {
            ...
            case .futureEvents:
                return .futureEvents
            case .allEvents:
                return .thisEvent // We handle "all" by removing the entire series
            }
        }
        ...
        var localizedTitle: String {
            switch self {
            case .thisEvent:
                return NSLocalizedString("event.delete.scope.this_event", comment: "This Event")
            case .futureEvents:
                return NSLocalizedString("event.delete.scope.future_events", comment: "Future Events")
            case .allEvents:
                return NSLocalizedString("event.delete.scope.all_events", comment: "All Events")
            }
        }
    }
    ...
    ///   - presentScopeSelection: Closure to present scope selection for recurring events (returns selected scope or nil if cancelled)
    ...
    func deleteEvent(
        ...
        return await deleteEventWithConfirmation(event, presentConfirmation: presentConfirmation, presentScopeSelection: presentScopeSelection)
    }
    ...
    private func deleteReminder(
        ...
        let message = NSLocalizedString("event.delete.confirm_message_reminder", comment: "Are you sure you want to delete this reminder?")
    ...
    private func deleteEventWithConfirmation(
        ...
        if isRecurring {
            switch scope {
            case .thisEvent:
                message = NSLocalizedString("event.delete.confirm_message_this", comment: "Delete this occurrence of the recurring event?")
            case .futureEvents:
                message = NSLocalizedString("event.delete.confirm_message_future", comment: "Delete this and all future occurrences?")
            case .allEvents:
                message = NSLocalizedString("event.delete.confirm_message_all", comment: "Delete all occurrences of this event?")
            }
        } else {
            message = NSLocalizedString("event.delete.confirm_message_single", comment: "Are you sure you want to delete this event?")
        }
...
enum EventDeletionError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notAuthorized:
            return NSLocalizedString("event.delete.error.not_authorized", comment: "Calendar access is required to delete events.")
        case .eventNotFound:
            return NSLocalizedString("event.delete.error.not_found", comment: "This event could not be found. It may have been deleted already.")
        case .typeMismatch:
            return NSLocalizedString("event.delete.error.type_mismatch", comment: "Invalid event type.")
        }
    }
}
