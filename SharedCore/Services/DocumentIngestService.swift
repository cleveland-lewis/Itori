...
class DocumentIngestService: DocumentIngestPort {
    ...
    private func extractPlainText(from url: URL, fileType: DocumentFileType) async throws -> (String, DocumentMetadata) {
    ...
    private func extractRTF(from url: URL) async throws -> (String, DocumentMetadata) {
        #if canImport(AppKit)
        guard let attributedString = NSAttributedString(rtf: try Data(contentsOf: url), documentAttributes: nil) else {
